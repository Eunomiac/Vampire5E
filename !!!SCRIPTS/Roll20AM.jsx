...
//		log('  >Tracks updated<');
    ...
    // Extracts the command details from a command string passed from handleInput	
	    ...
	    _.each(command.replace(`${cmdSep.action},`,"").split(","),(d)=>{
                consts=d.match(/(volTick|time|volume|delay|level|menu|mode|restrict|tag|API|access|tag|tag1|tag2|tag3|tag4|tag5|set|unset|filter|delayed|viewBy|display|)(?::|=)([^,]+)/) || null
                if(consts)
                    [,,cmdSep.details[consts[1]]]=consts
                else
                    cmdSep.details[d]=d
	...
	// changeHandler invoked when a track finishes in Jukebox.  
        ...
        listHandler = (cmdDetails,list,who,restrict) =>{
            if (debug){
    ...
    //* *****************************************************************************    
        ...
        // refresh the list constiable to get current playing track
	...
	// plays tracks randomly
        playListTogether = (list,startVolume,who) =>{
            ...
            list.playing = true
            // Set start volume
            const volume = startVolume === 0 ? startVolume : list.volume
    ...
    // fade in starts at zero an incrementally increases volume to the fade in volume
        fadeInTrack = (trackID,target,seconds,who) =>{
            let level
            const jbTrack = getJukeBox(trackID),
                trackDetails = getTrackDetails(trackID),
    ...
    //* *****************************************************************************
        ...
        increaseTrack = (trackID) =>{
            const trackDetails = getTrackDetails(trackID),
                jbTrack = getJukeBox(trackID),
    ...
    // checkListDuplicates = (listName,trackID,trackTitle,who) =>{
    //     const unique,trackCheck,titleCheck
...
// 	getTracks = (trackNames,restrict,ignore) =>{
// 	    return filterObjs((o)=>{
