...
const Complications = (() => {
        ...
        switch (call) {
            case "deckcheck": {
                ...
                for (const card of CARDS)
                    doomReportStrings.push(
                        `<b>[${card.rarity}] ${card.displayName}</b> ${getCardQtyForDeck(card)} ${
                            isCardValidForDeck(card) ? "IN" : "EXCLUDED"
                        }`
                    );
            ...
            case "make": {
                switch (D.LCase((call = args.shift()))) {
                    case "dragpads": {
                        ...
                        for (let i = 1; i <= 10; i++) DragPads.MakePad(`CompSpot_${i}`, "flipComp", {deltaWidth: -75, deltaHeight: -90});
                        break;
                    }
                    // no default
                }
            ...
            case "get": {
                if (args.length)
                    D.Alert(
                        [Object.keys(STATE.REF.MAT[D.Int(args[0] - 1)]).join(", "), D.JS(STATE.REF.MAT[D.Int(args[0] - 1)])].join("<br>")
                    );
                else D.Alert(D.JS(STATE.REF.MAT));
                break;
            }
            case "set": {
            ...
            case "reset":
                ...
                if (args[0]) setCompVals("target", D.Int(args.shift()));
                break;
            case "discard": {
                switch (D.LCase((call = args.shift()))) {
                    case "random": {
                        const randomSpot = getRandomSpot(["faceUp", "noLastDrawn"]);
                        if (VAL({number: randomSpot})) setCard(randomSpot, "discard");
                        break;
                    }
                    case "last": {
                        if (VAL({number: STATE.REF.lastDraw})) setCard(STATE.REF.lastDraw, "discard");
                        break;
                    }
                    default: {
            ...
            case "enhance": {
                switch (D.LCase((call = args.shift()))) {
                    case "random": {
                        ...
                        if (VAL({number: randomSpot})) setCard(randomSpot, "enhanced");
                        break;
                    }
                    case "last": {
                        if (VAL({number: STATE.REF.lastDraw}))
                            if (STATE.REF.MAT[STATE.REF.lastDraw].isEnhanced) setCard(STATE.REF.lastDraw, "!enhanced");
                            else setCard(STATE.REF.lastDraw, "enhanced");
                        break;
                    }
                    default: {
                        if (STATE.REF.MAT[D.Int(call) - 1].isEnhanced) setCard(D.Int(call) - 1, "!enhanced");
                        else setCard(D.Int(call) - 1, "enhanced");
                        break;
                    }
                }
                break;
            ...
            case "confirm": {
                switch (D.LCase((call = args.shift()))) {
                    case "last": {
                        if (VAL({number: STATE.REF.lastDraw})) setCard(STATE.REF.lastDraw, "confirm");
                        break;
                    }
                    default: {
            ...
            case "negate": {
                switch (D.LCase((call = args.shift()))) {
                    case "random": {
                        ...
                        if (VAL({number: randomSpot})) setCard(randomSpot, "negated");
                        break;
                    }
                    case "last": {
                        if (VAL({number: STATE.REF.lastDraw}))
                            if (STATE.REF.MAT[STATE.REF.lastDraw].isNegated) setCard(STATE.REF.lastDraw, "!negated");
                            else setCard(STATE.REF.lastDraw, "negated");
                        break;
                    }
                    default: {
                        if (STATE.REF.MAT[D.Int(call) - 1].isNegated) setCard(D.Int(call) - 1, "!negated");
                        else setCard(D.Int(call) - 1, "negated");
                        break;
                    }
                }
                break;
            ...
            case "duplicate": {
                switch (D.LCase((call = args.shift()))) {
                    case "random": {
                        ...
                        if (VAL({number: randomSpot})) dupeCard(randomSpot);
                        break;
                    }
                    case "last": {
                        if (VAL({number: STATE.REF.lastDraw})) dupeCard(STATE.REF.lastDraw);
                        break;
                    }
                    default: {
            ...
            case "revalue": {
                switch (D.LCase((call = args.shift()))) {
                    case "random": {
                        const randomSpot = getRandomSpot(["faceUp", "noLastDrawn"]);
                        if (VAL({number: randomSpot})) promptCardVal(randomSpot);
                        break;
                    }
                    case "last": {
                        if (VAL({number: STATE.REF.lastDraw})) promptCardVal(STATE.REF.lastDraw);
                        break;
                    }
                    default: {
    ...
    const CARDS = [
        {
            name: "AMatterOfPride",
            displayName: "A Matter of Pride",
            category: null,
            value: 1,
            rarity: "C",
            afterAction: (charRef, spot, isEnhanced) => {
                Char.Damage(charRef, "willpower", "superficial+", 2);
                if (isEnhanced) Char.Damage(charRef, "willpower", "aggravated", 1);
            }
        },
        {
            name: "AMomentOfDespair",
            displayName: "A Moment of Despair",
            category: null,
            value: 1,
            rarity: "C",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "AMomentOfInsight",
            displayName: "A Moment of Insight",
            category: "benefit",
            value: 1,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "AMomentOfInspiration",
            displayName: "A Moment of Inspiration",
            category: "benefit",
            value: 1,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "AtCrossPurposes",
            displayName: "At Cross Purposes",
            category: "project",
            value: 1,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "Cathexis",
            displayName: "Cathexis",
            category: "attention",
            value: 2,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
                STATE.REF.endMessageQueue.push(
                    isEnhanced ? "Confront an NPC of the Storyteller's choice!" : "Confront an NPC of your sect!"
                );
            }
        },
        {
            name: "CognitiveDissonance",
        ...
        },
        {
            ...
            action: (charRef, spot) => {
                ...
                else cardAlert(spot, "Draw, then choose to keep it or discard it.");
        ...
        {
            name: "GuiltByAssociation",
            displayName: "Guilt by Association",
            category: "advantage",
            value: 1,
            rarity: "C",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "HeavyLiesTheCrown",
            displayName: "Heavy Lies The Crown",
            category: "debilitation",
            value: 2,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
                STATE.REF.endMessageQueue.push(`Choose a player to spend Willpower with you${isEnhanced ? "..." : "."}`);
                if (isEnhanced) STATE.REF.endMessageQueue.push("... then choose again!");
            }
        },
        {
            name: "ImmortalClay",
            displayName: "Immortal Clay",
            category: "humanity",
            value: 2,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
                STATE.REF.endMessageQueue.push("Redesign one of your Convictions.");
                if (isEnhanced) STATE.REF.endMessageQueue.push("Redesign a Chronicle Tenet.");
            }
        },
        {
            name: "ImpendingDoom",
            ...
            action: (charRef, spot) => {
                ...
                cardAlert(
                    spot,
                    "Two copies of each non-Benefit Rare have been shuffled into the deck, and all face-down cards have been re-dealt."
                );
                buildDeck();
        ...
        {
            name: "IrresistibleOpportunity",
            displayName: "Irresistible Opportunity",
            category: "attention",
            value: 2,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "MentalBlock",
            displayName: "Mental Block",
            category: "debilitation",
            value: 3,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
                STATE.REF.endMessageQueue.push(
                    isEnhanced ? "Lose access to a chosen Discipline." : "Lose access to a chosen Discipline power."
                );
            }
        },
        {
            name: "Micromanagement",
            displayName: "Micromanagement",
            category: "blood",
            value: 2,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        },
        {
            ...
            action: (charRef, spot) => {
                ...
                if (STATE.REF.MAT[spot].isEnhanced) {
                    cardAlert(
                        spot,
                        "Draw twice, then discard one of them.<br>Reduce the value of the card you keep to zero.",
                        null,
                        " (Enhanced)"
                    );
        ...
        },
        {
            ...
            onEnhance: (charRef, spot) => {
                ...
                if (VAL({number: randomSpot})) {
                    ...
                    cardAlert(
                        spot,
                        ["Randomly enhancing a drawn Complication:", getCardName(randomSpot, true)].join("<br>"),
                        null,
                        " (Enhanced)"
                    );
        ...
        {
            name: "ProlongedAbsence",
            displayName: "Prolonged Absence",
            category: null,
            value: 2,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
                STATE.REF.endMessageQueue.push(
                    isEnhanced ? "Negate Domain Control until Project resolved." : "-1 Domain Control until Project resolved."
                );
            }
        },
        {
            name: "PyrrhicVictory",
            displayName: "Pyrrhic Victory",
            category: "project",
            value: 4,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "RecklessGamble",
            displayName: "Reckless Gamble",
            category: "complication",
            value: 0,
            rarity: "U",
            action: (charRef, spot) => {
                cardAlert(spot, ["Choose a Complication to negate.", "Then, a random Complication will be enhanced."].join("<br>"));
                ONNEXT.negated.push(() => {
                    const randomSpot = getRandomSpot(["noEnhanced", "noNegated", "faceUp", spot]);
                    if (VAL({number: randomSpot})) {
                        setCard(randomSpot, "enhanced");
                        cardAlert(spot, ["Randomly enhancing a drawn Complication:", getCardName(randomSpot, true)].join("<br>"));
                    }
                });
            },
            onEnhance: (charRef, spot) => {
        ...
        },
        {
            ...
            onEnhance: (charRef, spot) => {
                cardAlert(
                    spot,
                    ['If the Memoriam triggered by "Reverie" failed:', "Suffer a Total Failure."].join("<br>"),
                    null,
                    " (Enhanced)"
                );
            },
        ...
        {
            name: "SilentBeneficiary",
            displayName: "Silent Beneficiary",
            category: null,
            value: 1,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "TheBeastAscendant",
            displayName: "The Beast Ascendant",
            category: "beast",
            value: 2,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "TheBeastPerilous",
            displayName: "The Beast Perilous",
            category: "beast",
            value: 2,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "TheBeastDespotic",
            displayName: "The Beast Despotic",
            category: "beast",
            value: 2,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "TheBeastInsatiable",
            displayName: "The Beast Insatiable",
            category: "beast",
            value: 1,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "TheBeastInsensate",
            displayName: "The Beast Insensate",
            category: "beast",
            value: 2,
            rarity: "R",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "TheBeastRampant",
            displayName: "The Beast Rampant",
            category: "beast",
            value: 2,
            rarity: "C",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "TheBeastRavenous",
            displayName: "The Beast Ravenous",
            category: "beast",
            value: 2,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        {
            name: "TheBeastScorned",
            displayName: "The Beast Scorned",
            category: "beast",
            value: 2,
            rarity: "U",
            afterAction: (charRef, spot, isEnhanced) => {
        ...
        },
        {
            ...
            action: (charRef, spot) => {
                if (STATE.REF.MAT[spot].isEnhanced) {
                    ...
                    ONNEXT.confirm.push(...[newSpot => setCard(newSpot, "enhance"), newSpot => setCard(newSpot, "enhance")]);
                } else {
        ...
        // {name: "Triage", displayName: "", category: null, value: -1, rarity: ""},
        {
            ...
            action: (charRef, spot) => {
                ...
                if (VAL({number: randomSpot})) {
                    ...
                    cardAlert(spot, ["Devaluing a random drawn Complication:", getCardName(randomSpot, true)].join("<br>"));
                }
            },
            onEnhance: (charRef, spot) => {
                ...
                if (VAL({number: randomSpot})) {
                    revalueCard(randomSpot, 0);
                    cardAlert(spot, ["Devaluing your most valuable Complication:", getCardName(randomSpot, true)].join("<br>"));
                }
            }
        },
        {
            name: "UnderTheBus",
            displayName: "Under the Bus",
            category: null,
            value: 1,
            rarity: "U",
            action: (charRef, spot) => {
    ...
    const getCardQtyForDeck = card =>
        CARDQTYS[card.rarity] + CARDQTYS.overrides.map(x => (x.check(card) ? x.qty : 0)).reduce((tot, x) => tot + x, 0);
    const getActiveCards = () => _.filter(STATE.REF.MAT, v => isCardActive(v));
    const getUsedCategories = (omitSpots = []) =>
        _.uniq(_.compact(_.map(getActiveCards(), v => !omitSpots.includes(v.spot) && v.category)));
    const isCardValidForDeck = card =>
        VAL({list: card}) &&
        card.name &&
    ...
    const getCardName = (spot, isReturningFullName = false) =>
        (isReturningFullName && STATE.REF.MAT[spot].displayName) || STATE.REF.MAT[spot].name;
    const getRandomSpot = modes => {
        ...
        for (let i = 0; i < STATE.REF.MAT.length; i++) {
            ...
            for (const mode of modes) {
                switch (mode) {
                    case "maxValue":
                        if (card.value < Math.max(...STATE.REF.MAT.map(x => x.value))) isThisCardValid = false;
                        break;
                    case "isUndoable":
                        if (card.isNotUndoable) isThisCardValid = false;
                        break;
                    case "noEnhanced":
                        if (card.isEnhanced) isThisCardValid = false;
                        break;
                    case "noNegated":
                        if (card.isNegated) isThisCardValid = false;
                        break;
                    case "noLastDrawn":
                        if (i === STATE.REF.lastDraw) isThisCardValid = false;
                        break;
                    case "noDuplicated":
                        if (card.isDuplicated) isThisCardValid = false;
                        break;
                    case "faceUp":
                        if (!card.isFaceUp && !card.isDiscarded) isThisCardValid = false;
                        break;
                    default:
                        if (i === mode) isThisCardValid = false;
                        else if (VAL({string: mode}) && `${mode}`.startsWith("noValue") && card.value === D.Int(mode.replace(/\w/gu, "")))
                            isThisCardValid = false;
                        break;
                }
                if (!isThisCardValid) break;
            }
            if (isThisCardValid) validSpots.push(i);
        }
    ...
    const getCardStats = () => {
        ...
        for (const card of CARDS) cardData[card.rarity].push(card.name);
        D.Alert(
            [
    ...
    const setCard = (spot, mode, isShowingFX = true) => {
        ...
        const delayedCardIndices = DELAYQUEUE.map((x, i) => [x, i])
            .filter(x => VAL({array: x[0]}) && x[0][0].includes(onNextMode) && x[0][2] !== spot)
            .map(x => x[1]);
        const nextFuncIndex = ONNEXT[onNextMode]
            .map((x, i) => [x, i])
            .filter(x => VAL({function: x[0]}))
            .map(x => x[1])
            .shift();
        ...
        DB(
            ...
            `setCard${["replace", "faceDown"].includes(mode) ? mode : ""}`
        );

        if (mode === "discard") card.isDiscarded = true;
        ...
        if (mode !== "discard" && card.isDiscarded) return;
        ...
        switch (mode) {
            ...
            case "faceDown": {
                ...
                if (waitCallIndex >= 0) {
                    ...
                    DB(
                        `Waiting @ "FACEDOWN"<br>DELAYQUEUE: ${D.JS(DELAYQUEUE)}<br>ONNEXT[${onNextMode}]: ${D.JS(
                            ONNEXT[onNextMode]
                        )}<br>== <b>FINISHED [${mode}] <u>${getCardName(spot)}</u> @ ${spot}</b> ====`,
                        "setCard"
                    );
                    break;
                }
            ...
            case "faceUp": {
                ...
                if (waitCallIndex >= 0) {
                    ...
                    ONNEXT[onNextMode][waitCallIndex] = null;
                    DB(
                        `Waiting @ "FACEUP"<br>DELAYQUEUE: ${D.JS(DELAYQUEUE)}<br>ONNEXT[${onNextMode}]: ${D.JS(
                            ONNEXT[onNextMode]
                        )}<br>== <b>FINISHED [${mode}] <u>${getCardName(spot)}</u> @ ${spot}</b> ====`,
                        "setCard"
                    );
            ...
            case "confirm": {
                if (waitCallIndex >= 0) {
                    ...
                    ONNEXT[onNextMode][waitCallIndex] = null;
                    DB(
                        `Waiting @ "CONFIRM"<br>DELAYQUEUE: ${D.JS(DELAYQUEUE)}<br>ONNEXT[${onNextMode}]: ${D.JS(
                            ONNEXT[onNextMode]
                        )}<br>== <b>FINISHED [${mode}] <u>${getCardName(spot)}</u> @ ${spot}</b> ====`,
                        "setCard"
                    );
            ...
            case "activate": {
                ...
                if (card.isEnhanced) setCard(spot, "enhanced");
                if (waitCallIndex >= 0) {
                    DELAYQUEUE.push([...ONNEXT[onNextMode][waitCallIndex].split(":").slice(1), spot]);
                    ONNEXT[onNextMode][waitCallIndex] = null;
                    DB(
                        `Waiting @ "ACTIVATE"<br>DELAYQUEUE: ${D.JS(DELAYQUEUE)}<br>ONNEXT[${onNextMode}]: ${D.JS(
                            ONNEXT[onNextMode]
                        )}<br>== <b>FINISHED [${mode}] <u>${getCardName(spot)}</u> @ ${spot}</b> ====`,
                        "setCard"
                    );
            ...
            case "enhanced": {
                ...
                if (waitCallIndex >= 0) {
                    ...
                    ONNEXT[onNextMode][waitCallIndex] = null;
                    DB(
                        `Waiting @ "ENHANCED"<br>DELAYQUEUE: ${D.JS(DELAYQUEUE)}<br>ONNEXT[${onNextMode}]: ${D.JS(
                            ONNEXT[onNextMode]
                        )}<br>== <b>FINISHED [${mode}] <u>${getCardName(spot)}</u> @ ${spot}</b> ====`,
                        "setCard"
                    );
            ...
            case "negated": {
                ...
                if (waitCallIndex >= 0) {
                    ...
                    ONNEXT[onNextMode][waitCallIndex] = null;
                    DB(
                        `Waiting @ "NEGATED"<br>DELAYQUEUE: ${D.JS(DELAYQUEUE)}<br>ONNEXT[${onNextMode}]: ${D.JS(
                            ONNEXT[onNextMode]
                        )}<br>== <b>FINISHED [${mode}] <u>${getCardName(spot)}</u> @ ${spot}</b> ====`,
                        "setCard"
                    );
            ...
            default: {
                ...
                if (waitCallIndex >= 0) {
                    ...
                    ONNEXT[onNextMode][waitCallIndex] = null;
                    DB(
                        `Waiting @ "DEFAULT"<br>DELAYQUEUE: ${D.JS(DELAYQUEUE)}<br>ONNEXT[${onNextMode}]: ${D.JS(
                            ONNEXT[onNextMode]
                        )}<br>== <b>FINISHED [${mode}] <u>${getCardName(spot)}</u> @ ${spot}</b> ====`,
                        "setCard"
                    );
        ...
        if (nextFuncIndex || nextFuncIndex === 0) {
            ...
            if (VAL({function: nextFunc})) nextFunc(spot);
            DB(`@@@@@@@@<br><b><u>FINISHED NEXT FUNCTION for ${getCardName(spot)} at ${mode} @@@@`, "setCard");
        }
        for (const allFunc of allFuncs) allFunc(spot);
        ...
        if (nextMode) setCard(spot, nextMode);
    };
    const buildDeck = () => {
        STATE.REF.DECK = [];
        ...
        for (let i = 0; i < validCards.length; i++) {
            const qty = Math.max(
                0,
                getCardQtyForDeck(validCards[i]) - _.filter(STATE.REF.DISCARDS, v => v.name === validCards[i].name).length
            );
            for (let ii = 0; ii < qty; ii++) STATE.REF.DECK.push(validCards[i]);
        }
        Media.SetText("CompCardsRemaining", `${STATE.REF.DECK.length}`);
        Media.SetText(
            "CompCardsExcluded",
            `${STATE.REF.totalCards - STATE.REF.DECK.length - STATE.REF.DISCARDS.length - STATE.REF.MAT.filter(x => x.isFaceUp).length}`
        );
    ...
    const refreshDraws = (isShowingFX = true, isForcingRefresh = false) => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            else if (isForcingRefresh || !isCardValidForDeck(card)) setCard(i, "replace", isShowingFX);
    ...
    const resetComplication = (isRefreshing = true, startVal = 0) => {
        ...
        if (isRefreshing) refreshDraws(false);
    };
    ...
    const endComplication = (isLaunchingProject, isQueingActions = true) => {
        ...
        if (isQueingActions)
            for (const card of getActiveCards().filter(x => x.afterAction)) {
                ...
                if (card.isDuplicated) D.Queue(card.afterAction, [STATE.REF.charRef, card.spot, card.isEnhanced], "Comp", 0.5);
            }
                ...
                [
                    ...
                    C.HTML.Block(
                        [
                            ...
                            C.HTML.Body(
                                [
                                    `Launch Margin: ${(margin >= 0 && "+") || "-"}${Math.abs(
                                        margin
                                    )}&nbsp;&nbsp;&nbsp;&nbsp;Project Scope: ${scope}`,
    ...
    const sendEndMsgQueue = charRef => {
        ...
        if (STATE.REF.endMessageQueue.length) {
            ...
            for (const message of STATE.REF.endMessageQueue)
                if (message.charAt(0) === "!") messageLines.push(C.HTML.Body(message.slice(1), {color: C.COLORS.green}));
                else messageLines.push(C.HTML.Body(message));
    ...
    const sendGMPanel = () => {
        ...
        const getColor = (action, spot) => {
            ...
            switch (action) {
                case "discard":
                    if (card.isFaceUp) return C.COLORS.brightred;
                    break;
                case "enhance":
                    if (card.isFaceUp && !card.isEnhanced) return C.COLORS.brightpurple;
                    else if (card.isEnhanced) return C.COLORS.grey;
                    break;
                case "negate":
                    if (card.isFaceUp && !card.isNegated) return C.COLORS.blue;
                    else if (card.isNegated) return C.COLORS.brightgrey;
                    break;
                case "revalue":
                    if (card.isFaceUp) return C.COLORS.darkgreen;
                    break;
                case "duplicate":
                    if (card.isFaceUp && !card.isDuplicated) return C.COLORS.midgold;
                    else if (card.isDuplicated) return C.COLORS.brightgrey;
                    break;
                // no default
            }
        ...
        for (const actionType of ["discard", "enhance", "negate", "revalue", "duplicate", "confirm"])
            subPanels.push({
                ...
                rows: [
                    ...
                    {
                        type: "ButtonLine",
                        contents: [1, 2, 3, 4, 5].map(x => ({
                            name: x,
                            command: `!comp ${actionType} ${x}`,
                            styles: {width: "16%", bgColor: getColor(actionType, x - 1)}
                        })) // , fontSize: "10px", bgColor: C.COLORS.purple, buttonTransform: "none"}}))
                    },
                    {
                        type: "ButtonLine",
                        contents: [6, 7, 8, 9, 10].map(x => ({
                            name: x,
                            command: `!comp ${actionType} ${x}`,
                            styles: {width: "16%", bgColor: getColor(actionType, x - 1)}
                        })) // , fontSize: "10px", bgColor: C.COLORS.purple, buttonTransform: "none"}}))
        ...
        D.CommandMenu(
            {
                title: "Complications Control",
                rows: [
                    {type: "ButtonLine", contents: [{name: "Unlock", command: "!comp unlock"}]},
                    ..._.values(_.groupBy(subPanels, (x, i) => Math.floor(i / 2))).map(x => ({
                        type: "Column",
                        contents: x,
                    ...
                    })),
                    {
                        type: "ButtonLine",
                        contents: [
                            {name: "Finish", command: "!comp end false true"},
                            {name: "Launch!", command: "!comp end true true"}
                        ]
                    }
                ]
            },
            null,
            "ComplicationsControl"
        );
        // }
    };
