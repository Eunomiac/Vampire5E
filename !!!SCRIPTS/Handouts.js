...
const Handouts = (() => {
    ...
    const makeHandoutObj = (title, category, contents, isRawCode = false) => {
        if (category)
            ...
            delHandoutObjs(title);
        const noteObj = createObj("handout", {name: `${title} ${category && getCount(category) > 1 ? getCount(category) - 1 : ""}`.trim()});
        if (contents)
    ...
    const delHandoutObjs = (titleRef, category) => {
        for (const handout of _.filter(findObjs({_type: "handout", archived: false}), v => D.LCase(titleRef) && D.LCase(v.get("name")) && D.LCase(v.get("name")).startsWith(D.LCase(titleRef))))
            handout.remove();
        if (category)
            STATE.REF.noteCounts[category] = 0;
    };
    const delHandoutObj = (titleRef, category) => {
        const handoutObj = _.find(findObjs({_type: "handout", inplayerjournals: "", archived: false}), v => D.LCase(titleRef) && D.LCase(v.get("name")) && D.LCase(v.get("name").replace(/\s*\d*$/gu, "")) === D.LCase(titleRef));
        if (VAL({object: handoutObj})) {
