...
const Chat = (() => {
                    ...
                    default: {
                        ...
                        switch (call) {
                            ...
                            case "statewrite": {
                                switch (D.LCase(call = args.shift())) {
                                    case "all": {
                                        const isFormatting = args[0] === "parse";
                                        args = args[0] === "parse" ? args.slice(1) : args;
                                        for (const stateKey of Object.keys(state[C.GAMENAME]))
                                            if (state[C.GAMENAME][stateKey] && !_.isEmpty(state[C.GAMENAME][stateKey]))
                                                getStateData([stateKey], undefined, true, isFormatting);
                                        break;
                                    }
                                    case "parse": {
                                        getStateData(args, undefined, true);
                                        break;
                                    }
                                    default: {
                                        args.unshift(call);
                                        if (!getStateData(args, undefined, true, false))
                                            sendHelpMsg();
                                        break;
                                    }
                                }
    ...
    const sendHelpMsg = () => { D.Alert("Syntax Failure.", "CHAT.API")};
    // #endregion
    ...
    const getStateData = (namespace, returnVals, isWriting = false, isFormatting = true) => {
        D.DBAlert({namespace, returnVals, isWriting, isFormatting}, "getStateData", "Chat");
        let [stateInfo, isVerbose] = [state, false];
        isVerbose = namespace[0] === "full" || !isFormatting;
        if (namespace[0] === "full")
            namespace.shift();
        ...
        const title = `${isWriting ? D.UCase(namespace.slice(1).join(".")) : `state.${namespace.join(".")}`}${isWriting && !isFormatting ? " (Backup)" : ""}`;
        while (namespace && namespace.length)
            stateInfo = stateInfo[namespace.shift()];
        if (returnVals) {
            const returnInfo = {};
            _.each(stateInfo, (data, key) => {
                if (isFormatting && ALERTBLACKLIST.includes(key)) {
                    ...
                    returnInfo[key] = D.Clone(returnData);
                }
            });
            if (isWriting) {
                Handouts.Make(title, false, returnInfo, !isFormatting);
                D.Alert(`Wrote ${title} to Handout`, "GetStateData");
            } else {
            ...
            if (isFormatting) {
                if (_.isEmpty(stateInfo))
                    return D.Alert(`State data for ${title} is empty.`, "getStateData");
                stateInfo = D.JS(D.KeyMapObj(stateInfo, undefined, (v, k) => ALERTBLACKLIST.includes(k) && "<b><u>(HIDDEN)</u></b>" || v), isVerbose);
            } else {
                stateInfo = _.escape(JSON.stringify(stateInfo));
            }
            if (isWriting) {
                Handouts.Make(title, false, stateInfo, true);
                D.Alert(`Wrote ${title} to Handout`, "GetStateData");
            } else {
                D.Alert(stateInfo, title);
            }
        }
        return true;
    };
