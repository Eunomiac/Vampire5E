...
const Roller = (() => {
    ...
    const setDieCat = (dieCat, dieVals = [], rollType = "trait") => {
            ...
            switch (name) {
                ...
                case "outcome": {
                    switch (rollData.type) {
                        ...
                        case "trait": {
                            if (!rollResults.noBestialFail && rollResults.H.botches > 0 && (rollResults.total === 0 || D.Int(rollResults.margin) < 0)) {
    ...
    */
    ...
    const getResonance = (charRef, posRes = "", negRes = "", /* marginBonus = 0, */ isDoubleAcute, testCycles = 0) => {
        ...
        if (D.Int(testCycles) > 0) {
            ...
            const flaResults = Object.keys(record).slice(4).map(k => [k.slice(0,1), parseFloat(record[k].PER.slice(0,-1))]).sort((a,b) => b[1] - a[1]).map(x => `${x[0]}: ${x[1]}%`).join(", ");
    ...
    return {
        ...
        get DiceCats() { return Object.keys(SETTINGS.dice) },
