...
const SoundScape = (() => {
    ...
    // #region LOCAL INITIALIZATION
        initialize = () => {
            ...
            STATE.REF.nomenu = STATE.REF.nomenu || "On"            
        }
    ...
    // #region Chat Menu Creation
        ...
        outputConfig = (menu, who) => {
            let mvTextButton, backButton, output, outputWho
            if (STATE.REF.API == "gm")
                outputWho = "/w gm"
            else if (STATE.REF.API == "All")
                outputWho = " "
            else
                outputWho = `/w ${STATE.REF.API}`
            ...
            if (menu) {
                ...
                output += `<div>${mvTextButton}</div>`
                // Access Button
                mvTextButton = makeButton("!roll20AM --config,access=?{Access|None,None|Limited,Limited|Full,Full} --config", STATE.REF.access, "white", "black")
                output += `<b>Access: </b><div style="display:inline-block">${mvTextButton}</div><br>`
                // Access Button
                mvTextButton = makeButton("!roll20AM --config,display=?{Display|On,On|Off,Off} --config", STATE.REF.nomenu, "white", "black")
            ...
            output += "</div>"
            if (menu) {
                if (STATE.REF.menu != "nomenu" && STATE.REF.nomenu == "On") {
                    sendChat(who, outputWho + output, null, {noarchive: true})
                    // if SoundScape is configured to send menus to a player, then send again to GM
                    if (STATE.REF.API != "All" && STATE.REF.API != "gm")
    ...
    // #region Handlers (Chat, Jukebox)
        ...
        inputHandler = (origMsg) => { // INVOKED on !roll20AM chat commands
            ...
            if (origMsg.content.indexOf("!roll20AM") !== 0)
                return
            const listHandler = (cmdDetails, list, who, restrict) => {
                    DB([
                    ...
                    if (restrict && list.access == "gm") {
                        sendChat(who, "Invalid SoundScape Command. Restricted action not allowed by a player.", null, {noarchive: true})
                        return
                    }
                ...
                // play depending on mode 
                    if (cmdDetails.details.play) {
                        ...
                        increaseListVolume(list, who)
                    } else if (cmdDetails.details.decrease) {
                        decreaseListVolume(list, who)
                    } else {
                        showHelp(who)
                    }
                },
                // handles playList actions, start/stop/fadein/fadeout
                trackHandler = (cmdDetails, trackDetails, who, restrict) => {
                    DB([
                    ...
                    if (cmdDetails.details.increase)
                        increaseTrackVolume(trackDetails.id, who)

                    if (cmdDetails.details.decrease)
                        decreaseTrackVolume(trackDetails.id, who)

                },
                configHandler = (cmdDetails, who) => {
            ...
            if (args[0] === "!roll20AM")
                if (args[1]) {
                    _.each(_.rest(args, 1), (cmd) => {
                        ...
                        if (cmdDets.details.nomenu)
                            STATE.REF.menu = cmdDets.details.nomenu
                                ...
                                if (cmdDets.action == "audio") {
                                    if (playLists.length > 0)
                                        _.each(playLists, (listName) => {
                                            listHandler(cmdDets, getPlayListData(listName), who, restrict)
                                        })

                                    if (tracklists.length > 0)
                                        _.each(tracklists, (trackTitle) => {
                                            trackHandler(cmdDets, getTrackData(getTrackID(trackTitle)), who, restrict)
                                        })
                                    ...
                                    if (playLists.length > 0)
                                        _.each(playLists, (listName) => {
                                            editHandler(cmdDets, getPlayListData(listName), null, who, restrict)
                                        })

                                    if (tracklists.length > 0)
                                        _.each(tracklists, (trackTitle) => {
                                            editHandler(cmdDets, null, getTrackData(getTrackID(trackTitle)), who, restrict)
                                        })

                                    if (tracklists.length == 0 && playLists.length == 0)
                                        editHandler(cmdDets, null, null, who, restrict)

                                    // config commands    
                                } else if (cmdDets.action == "config") {
                                    configHandler(cmdDets, who, restrict)
                                } else {
                                    sendChat(who, "Invalid SoundScape Command.  Valid commands are --audio, --edit, --config", null, {noarchive: true})
                                }
            ...
            if (obj.get("softstop") === true && prev.softstop === false) {
                ...
                if (!listFound) {
                    ...
                    if (trackDetails.mode == "loop")
                        ...
                        outputConfig(STATE.REF.menu, trackDetails.who)
    ...
    // #endregion
    // #region Config Menu Commands
        // Import everything from Jukebox
        removeJukebox = (who) => {
            STATE.REF.playLists = {}
            STATE.REF.trackDetails = {}
            sendChat(who, "All PlayLists and Tracks have been removed from SoundScape", null, {noarchive: true})
        },
        // Import everything from Jukebox
        importJukebox = (who) => {
        ...
        showHelp = (who) => {
            sendChat("SoundScape", `/w "${who}"`
                    ...
                    + `SoundScape v${version} -- Hear the dice, hear the action!`
                    + "</div></div>"
                    + `For all the details on SoundScape, please see the <u><b>[SoundScape handout](${helpLink})</b></u>.`, null, {noarchive: true}
            )
        },
    // #endregion

    //* **** DATA RETRIEVAL & CONVERSION ********************************************
    // #region GETTERS: Tracks
        getTrackKey = (trackRef) => {
            if (VAL({string: trackRef})) {
                ...
                return getTrackKey(trackRef.id)
            }
            return false
        },
        isTrack = (trackRef) => Boolean(getTrackKey(trackRef)),
        ...
        getTrackID = (trackRef) => {
            ...
            return VAL({list: trackData}) && trackData.id
        },
        getTrackObj = (trackRef) => getObj("jukeboxtrack", getTrackID(trackRef)),
    // #endregion
    // #region GETTERS: PlayLists
        /* PLAYLIST MODES: {
        ...
        isPlayList = (playListRef) => Boolean(getPlayListKey(playListRef)),
        getPlayListData = (playListRef) => {
            const playListKey = getPlayListKey(playListRef)
            return VAL({string: playListKey}) && STATE.REF.playLists[playListKey]
        },
        getPlayListTracks = (playListRef) => {
            const playListData = getPlayListData(playListRef)
            return VAL({list: playListData}) && playListData.trackids.map(x => getTrackKey(x))
        },
        getPlayListMode = (playListRef) => {
            const playListData = getPlayListData(playListRef)
            return VAL({list: playListData}) && playListData.mode
        },
        isPlayListLooping = (playListRef) => ["loop", "randomLoop", "shuffle"].includes(getPlayListMode(playListRef)),
        isTrackInPlayList = (trackRef, playListRef) => {            
            const trackID = getTrackID(trackRef),
                playListData = getPlayListData(playListRef)
            return VAL({list: playListData}) && playListData.trackids.includes(trackID)
        },
        isOneTrackPlaying = (playListRef) => { /* Returns "true" if there is exactly one track from the playlist currently playing. */ },
    // #endregion
    ...
    // #region Playing & Stopping Tracks
        playTrack = (trackID, level, loop, who) => {
            ...
            DB({track: getTrackKey(trackID), volume: level}, "playTrack")
            _.each(STATE.REF.playLists, list => {
    ...
    // #region Fade Control
        ...
        fadeInTrack = (trackID, target, seconds, who) => {
        ...
        fadeOutTrack = (trackID, level, target, seconds, nomenu, who) => {
            const jbTrack = getTrackObj(trackID), trackDetails = getTrackData(trackID)

                // verify that the list is actualy playing
            if (!trackDetails.playing) {
                sendChat("SoundScape", `/w "${who}" In order to fade a track, it must be playing.`, null, {noarchive: true})
                return
            }
            DB({track: getTrackKey(trackID), target, time: seconds}, "fadeOutTrack")
                // calc the volume change per second
            ...
            const timer = setInterval(() => {
                ...
                if (Number(level) > target) {
                    ...
                    clearTimeout(timer)
                    if (!nomenu)
                        outputConfig(STATE.REF.menu, who)
    ...
    // #region Configure Default Volume, Fade, Delay
        ...
        changeVolumeTrack = (trackID, level) => {
            const trackDetails = getTrackData(trackID)
            DB({track: getTrackKey(trackID), volume: level}, "changeVolumeTrack")
            trackDetails.volume = level
        },
        changeFadeInTrack = (trackID, level) => {
            const trackDetails = getTrackData(trackID)
            DB({track: getTrackKey(trackID), fadeup: level}, "changeFadeInTrack")
            trackDetails.fadeup = level
        },
        changeFadeOutTrack = (trackID, level) => {
            const trackDetails = getTrackData(trackID)
            DB({track: getTrackKey(trackID), fadedown: level}, "changeFadeOutTrack")
            trackDetails.fadedown = level
        },
        changeFadeTimeTrack = (trackID, level) => {
            const trackDetails = getTrackData(trackID)
            DB({track: getTrackKey(trackID), fadetime: level}, "changeFadeTimeTrack")
            trackDetails.fadetime = level
        },
        changeDelayTrack = (trackID, level) => {
            const trackDetails = getTrackData(trackID)
            DB({track: getTrackKey(trackID), delay: level}, "changeDelayTrack")
    ...
    // #region Playing & Stopping PlayLists
        ...
        playListInOrder = (list, startVolume, who) => {
        ...
        playListRandom = (list, startVolume, who) => {
        ...
        // plays tracks randomly
        playListShuffle = (list, startVolume, who) => {
        ...
        // plays tracks randomly
        playListTogether = (list, startVolume, who) => {
        ...
        // stops the playList
        stopList = (list, who) => {
            DB(`Stopping:${list.name}`, "stopList")
    ...
    // #region Volume Control
        increaseListVolume = (list, who) => {
            ...
            _.each(list.trackids, (trackID) => {
                increaseTrackVolume(trackID, who)
            })
        },
        decreaseListVolume = (list, who) => {
            ...
            _.each(list.trackids, (trackID) => {
                decreaseTrackVolume(trackID, who)
            })
        },
    // #endregion   
    // #region Fade Control
        ...
        fadeListOut = (list, nomenu, who) => {
            let trackDetails
                // verify that the list is actualy playing
            if (!list.playing) {
                sendChat("SoundScape", `/w "${who}" In order to fade a list, it must be playing.`, null, {noarchive: true})
                return
            }
    ...
    // #region Loop Tracking
        getJukeboxTracks = () => { /* Should return ACTUAL tracks currently playing on the Jukebox. */ },
        getJukeboxLoops = () => { /* Filters the above for tracks that are looping *OR* tracks that are part of a playlist that is looping. */ },
        registerLoop = (songRef) => { /* Any playlist or track activated here that is looping should be registered. */ },
        unregisterLoop = (songRef) => { /* The reverse of the above: when a track/playlist is no longer supposed to be looping. */ },
    // #endregion
