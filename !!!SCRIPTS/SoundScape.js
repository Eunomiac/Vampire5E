...
const Soundscape = (() => {
    ...
    const playSound = (soundRef, masterSound) => { // Initializes any playlist as if playing it for the first time. To preserve sequences, use playNextSound()
    ...
    const playNextSound = (trackRef) => { // Playlist must have been initialized for sequencing: First play should be with playSound()
        if (STATE.REF.isSoundscapeActive) {
