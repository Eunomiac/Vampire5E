...
const Session = (() => {
    ...
    const initialize = () => {
        ...
        if (!STATE.REF.locationRecord) {
            ...
            for (const mode of Session.Modes) STATE.REF.locationRecord[mode] = D.Clone(STATE.REF.curLocation);
        }
        setPlayerPage();
                ...
                switch (D.LCase((call = args.shift()))) {
                    ...
                    case "pointer": {
                        ...
                        if (pointer.layer === "map") {
                            ...
                            D.Alert(
                                `Setting pointer position for <b>"${siteName || siteRef}"</b>
                                
                                Move the map indicator to the desired position, then type "!sess set pointer" again.`,
                                "!sess set pointer"
                            );
                        }
                        break;
        ...
        changeMode("Active", true, [
            [
                ...
                [
                    "all",
                    C.HTML.Block([
                        C.HTML.Title("VAMPIRE: TORONTO by NIGHT", {
                            fontSize: "28px"
                        }),
                        C.HTML.Body("Initializing Session...", {
                            margin: "0px 0px 10px 0px"
                        }),
                        C.HTML.Header(`Welcome to Session ${D.NumToText(STATE.REF.SessionNum, true)}!`),
                        C.HTML.Body("Clock Running.<br>Animations Online.<br>Roller Ready.", {
        ...
        if (
                ...
                [
                    ...
                    [
                        ...
                        C.HTML.Block([
                            ...
                            C.HTML.Title("See you next week!", {
                                fontSize: "32px"
                            })
                        ])
                    ]
            ...
            if (!STATE.REF.isTestingActive || STATE.REF.isFullTest) {
                ...
                for (const char of D.GetChars("registered"))
                    ...
                    else Char.AwardXP(char, 2, "Session XP award.");
    ...
    const logTokens = mode => {
        ...
        for (const tokenObj of tokenObjs)
            STATE.REF.tokenRecord[mode][tokenObj.id] = {
                ...
                src:
                    (
                        Media.TOKENS[D.GetName(tokenObj.get("represents"))] || {
                            curSrc: "base"
                        }
                    ).curSrc || "base"
            };
    ...
    const toggleFullTest = isFullTesting => {
        ...
        Media.SetTextData("testSessionNotice", {
            color: (isFullTesting && C.COLORS.brightred) || C.COLORS.puregreen
        });
        Media.SetTextData("testSessionNoticeSplash", {
            color: (isFullTesting && C.COLORS.brightred) || C.COLORS.puregreen
        });
        if (isFullTesting) {
            ...
            rows: [
                ...
                ..._.chain({
                    ["<<< Left"]: [
                        ...
                        {
                            width: "30%",
                            bgColor: (STATE.REF.sceneFocus === "l" && C.COLORS.gold) || C.COLORS.grey
                        }
                    ],
                    [">> Both <<"]: [
                        "!reply focus@center",
                    ...
                    ["Right >>>"]: [
                        ...
                        {
                            width: "30%",
                            bgColor: (STATE.REF.sceneFocus === "r" && C.COLORS.gold) || C.COLORS.grey
                        }
                    ]
                })
                    .mapObject((v, k) => (v ? {name: k, command: v[0], styles: v[1]} : 0))
    ...
    const getActiveLocations = (focusOverride, isIncludingSubLocs = true) => {
        ...
        if (VAL({string: focusOverride}))
            switch ({c: "Center", l: "Left", r: "Right", a: "All"}[focusOverride.toLowerCase().charAt(0)]) {
                case "Center": {
                    if (Object.keys(activeLocs).some(x => x.endsWith("Center"))) return _.omit(activeLocs, (v, k) => !k.endsWith("Center"));
                }
                /* falls through */
                case "Left":
        ...
        D.CommandMenu(
            ...
            commandString => {
                ...
                switch (commandString) {
                    case "reset": {
                        PENDINGLOCCOMMAND = D.Clone(BLANKPENDINGLOCCOMMAND);
                        Media.Notify("● Resetting pending location data.");
                        distCommandMenu();
                        break;
                    }
                    case "same": {
                        ...
                        Media.Notify(
                            `● Setting District${cmdIndex === 1 ? " RIGHT" : " (First)"} to Active District: "${getActiveDistrict()}"`
                        );
                    }
                    /* falls through */
                    default: {
                        PENDINGLOCCOMMAND.Districts[cmdIndex] = [commandString];
                        Media.Notify(`● District${cmdIndex === 1 ? " RIGHT" : " (First)"} set: "${D.UCase(commandString)}"`);
                        siteCommandMenu();
                        break;
                    }
                }
    ...
    const siteCommandMenu = () => {
        ...
        D.CommandMenu(siteMenuCode, commandString => {
            ...
            for (const [command, value] of Object.entries(params))
                switch (command) {
                    ...
                    case "done": {
                        ...
                        Media.Notify("● FINISHED! Setting Location Cards...");
                        return false;
                    }
                    case "name": {
                        if (VAL({array: PENDINGLOCCOMMAND.Sites[cmdIndex]})) {
                            PENDINGLOCCOMMAND.Sites[cmdIndex][1] = value;
                            Media.Notify(`● Site${cmdIndex === 1 ? " RIGHT" : " (First)"} Renamed to: "${value}"`);
                        } else {
                            Media.Notify("● Choose a Site before renaming!");
                        }
                        break;
                    }
                    case "focus": {
                        STATE.REF.sceneFocus = D.LCase(value.charAt(0));
                        Media.Notify(`● Scene Focus switched to: ${D.Capitalize(value)}`);
                        break;
                    }
                    case "site": {
                        switch (value) {
                            case "blank": {
                                ...
                                Media.Notify(`● Blanking Site${cmdIndex === 1 ? " RIGHT" : " (First)"}.`);
                                break;
                            }
                            case "same": {
                                params.site = getActiveSite(true);
                                Media.Notify(
                                    `● Setting Site${cmdIndex === 1 ? " RIGHT" : " (First)"} to Active Site: "${params.site[1] ||
                                        params.site[0]}"`
                                );
                            }
                            /* falls through */
            ...
            DB(
                {
                    [`PENDINGLOCCOMMAND After "!reply ${D.JSL(commandString)}":`]: PENDINGLOCCOMMAND
                },
                "siteCommandMenu"
            );
        ...
        D.CommandMenu(
            {
                ...
                rows: [
                    ...
                    {
                        type: "Column",
                        contents: [subLocPanels.Left, subLocPanels.Right],
                        style: {width: "47%", margin: "0px 1% 0% 1%"}
                    },
                    {
                        type: "Column",
        ...
        D.CommandMenu(
            {
                rows: _.compact([
                    {
                        type: "Header",
                        contents: `Current Focus: ${{c: "Center", l: "Left", r: "Right"}[STATE.REF.sceneFocus]}`
                    },
                    ..._.chain({
                        ["<<< Left"]: [
                            "!reply focus@left",
    ...
    const setModeLocations = (mode, isForcing = false) => {
        setLocation(STATE.REF.locationRecord[mode], null, isForcing);
    };
    ...
    const promptRemorseCheck = charObjs => {
        ...
        for (const charObj of charObjs)
            buttons.push({
                name: D.GetName(charObj, true),
                command: `!roll quick remorse ${charObj.id}`
            });
        D.CommandMenu({
            title: "Remorse Checks",
            rows: Object.values(_.groupBy(buttons, (v, i) => i % 2)).map(x => ({
                type: "ButtonLine",
                contents: x
            }))
        });
    ...
    const setSceneFocus = locPos => {
        ...
        DB(
            {
                locPos,
                ["state Scene Focus"]: STATE.REF.sceneFocus,
                activeLocs: getActivePositions()
            },
            "setSceneFocus"
        );
