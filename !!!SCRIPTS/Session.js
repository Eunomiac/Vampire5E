...
const Session = (() => {
            ...
            (commandString) => {
                ...
                switch (commandString) {
                    ...
                    case "same": {
                        ...
                        Media.Notify(`● Setting District${cmdIndex === 1 ? " RIGHT" : " (First)"} to Active District: "${getActiveDistrict()}"`);
                    }
                    /* falls through */
                    default: {
                        ...
                        case "site": {
                            switch (value) {
                                ...
                                case "same": {                                    
                                    ...
                                    Media.Notify(`● Setting Site${cmdIndex === 1 ? " RIGHT" : " (First)"} to Active Site: "${params.site[1] || params.site[0]}"`);
                                }
                                /* falls through */
                                default: {
                                    ...
                                    Media.Notify(`● Site${cmdIndex === 1 ? " RIGHT" : " (First)"} set: "${D.UCase(params.site[1] || params.site[0])}"`);
                                    break;
                                }
                            }
                            break;
                ...
                if ("done" in params) {
                    ...
                    for (const [locRef, subLocRef] of Object.entries(params))
                        if (locRef in BLANKLOCRECORD.subLocs) {
                            ...
                            Media.Notify(`● Sub-Location ${locRef} at ${PENDINGLOCCOMMAND.Sites[PENDINGLOCCOMMAND.workingIndex][0]} set: "${subLocRef}"`);
                        }
                    return C.REPLY.KEEPOPEN;
                }
            });  
    ...
    const setModeLocations = (mode, isForcing = false) => { setLocation(STATE.REF.locationRecord[mode], null, isForcing) };
