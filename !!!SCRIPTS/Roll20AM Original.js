...
var Roll20AM = Roll20AM || (function () {
    ...
    var version = "2.12",
        ...
        outputTrack = function (track, menu) {
            let title = track.title.split("by"), output, accessButton, playButton, modeButton, increaseButton, decreaseButton, fadeButton, fadeOutButton, fadeInButton, playlistButton, tagButton, deleteButton, mvTextButton
            if (track.playing)
        ...
        outputList = function (list, menu, mainMenu) {
            let output, accessButton, playButton, modeButton, increaseButton, decreaseButton, fadeButton, fadeOutButton, fadeInButton, playlistButton, tagButton, deleteButton, mvTextButton
            if (list.playing)
        ...
        inputHandler = function (msg_orig) { // INVOKED on !roll20AM chat commands
            // must be roll20AM to start all this
            if (msg_orig.content.indexOf("!roll20AM") !== 0)
                return
            const listHandler = function (cmdDetails, list, who, restrict) {
                    if (debug) {
                ...
                editHandler = function (cmdDetails, list, trackDetails, who, restrict) {
                    let mode, access, tag1, tag2, tag3, tag4, tag5
            ...
            let msg = _.clone(msg_orig), cmdDetails, args, restrict, player, who
                ...
                if (args[1]) {
                    _.each(_.rest(args, 1), (cmd) => {
                        ...
                        _.each(command.replace(`${cmdDetails.action},`, "").split(","), (d) => {
                            ...
                            if (vars)
                                cmdDetails.details[vars[1]] = vars[2]
                            else
                                cmdDetails.details[d] = d

                        })

                        cmdDetails.details.delay = cmdDetails.details.delay * 1000 || 0
                        if (debug)
                            log(cmdDetails)

                        let tracklists = [], playlists = [], timer, list
                                ...
                                if (cmdDetails.action == "audio") {
                                    if (playlists.length > 0)
                                        _.each(playlists, (listName) => {
                                            listHandler(cmdDetails, getListData(listName), who, restrict)
                                        })
                                    ...
                                    if (playlists.length > 0)
                                        _.each(playlists, (listName) => {
                                            editHandler(cmdDetails, getListData(listName), null, who, restrict)
                                        })
        ...
        changeHandler = function (obj, prev) { // INVOKED when a track finishes in Jukebox.  
        ...
        destroyHandler = function (obj) { // INVOKED when a jukeboxtrack is manually destroyed, removing it from Roll20AM
        ...
        // #region Config Menu Commands
            ...
            importJukebox = function (who) {
                let shuffleState = {"s": "shuffle", "a": "together", "b": "single"},
                    lists = JSON.parse(Campaign().get("jukeboxfolder")),
                    tagName, track, title, unique, trackID, trackDetails
            ...
            generateHelp = function () {
                const help = getObj("handout", state.Roll20AM.help),
        ...
        // #region Playing & Stopping Tracks
            playTrack = function (trackID, level, loop, who) {
                const jbTrack = getJukeBox(trackID), trackDetails = getTrackDetails(trackID)
        ...
        // #region Volume Control
            increaseTrackVolume = function (trackID, who) {
                let level, trackDetails = getTrackDetails(trackID), jbTrack = getJukeBox(trackID)
                if (debug)
                    log(`Increase Track:${trackDetails.title}`)

                level = Number(trackDetails.volume) + 5
                trackDetails.volume = level
                jbTrack.set({volume: level})
            },
            decreaseTrackVolume = function (trackID, who) {
                let level, trackDetails = getTrackDetails(trackID), jbTrack = getJukeBox(trackID)
        ...
        // #region Fade Control
            ...
            fadeInTrack = function (trackID, target, seconds, who) {
                let jbTrack = getJukeBox(trackID), trackDetails = getTrackDetails(trackID), levelChange, level, timer

                // calc the volume change per second
                level = 0
                levelChange = Number(target) / Number(seconds)
                if (debug) {
                    log(`Target:${target}`)
                    log(`Time:${seconds}`)
            ...
            fadeOutTrack = function (trackID, level, target, seconds, nomenu, who) {
                let jbTrack = getJukeBox(trackID), trackDetails = getTrackDetails(trackID), levelChange, timer
                ...
                // calc the volume change per second
                levelChange = (Number(level) - Number(target)) / seconds
                if (debug)
                    log(`Volume Change:${levelChange}`)
        ...
        // One function for each playlist mode:
        ...
        // ... plus the function that actually plays the track triggered by the above four functions
            playNextTrack = function (list, listTracks, startVolume, who) {
                if (debug)
                    log(`Playing Next Track List:${list.name}`)

                // initialize
                let found = false, nexttrackID, trackID, volume
        ...
        // #endregion
        // #region Volume Control
            increaseListVolume = function (list, who) {
                let level
            ...
            decreaseListVolume = function (list, who) {
                if (debug) {
                ...
                newVolume = Number(list.volume) - 5
