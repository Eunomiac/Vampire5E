...
const Media = (() => {
    ...
    // #region LOCAL INITIALIZATION
        initialize = () => {
            const funcID = ONSTACK()
            STATE.REF.animregistry.WeatherLightning_1.timeOut = 2000
            STATE.REF.animregistry.WeatherLightning_2.timeOut = 2000

            /* STATE.REF.animregistry.WeatherLightning_1.validModes = ["Active"]
            STATE.REF.animregistry.WeatherLightning_1.soundEffect = "Thunder"
            STATE.REF.animregistry.WeatherLightning_2.validModes = ["Active"]
            STATE.REF.animregistry.WeatherLightning_2.soundEffect = "Thunder" */

            STATE.REF.imgregistry.MapButton_Roads_1.srcs.off = "https://s3.amazonaws.com/files.d20.io/images/96594791/NKN-0QyHbcOXS2_o5X6Y2A/thumb.png?1573368252"
            STATE.REF.imgregistry.MapButton_Parks_1.srcs = "MapButton_Roads_1"
            STATE.REF.imgregistry.MapButton_Rack_1.srcs = "MapButton_Roads_1"
            STATE.REF.imgregistry.MapButton_SitesEducation_1.srcs = "MapButton_Roads_1"
            STATE.REF.imgregistry.MapButton_SitesHavens_1.srcs = "MapButton_Roads_1"
            STATE.REF.imgregistry.MapButton_SitesHealth_1.srcs = "MapButton_Roads_1"
            STATE.REF.imgregistry.MapButton_SitesLandmarks_1.srcs = "MapButton_Roads_1"
            STATE.REF.imgregistry.MapButton_SitesNightlife_1.srcs = "MapButton_Roads_1"
            STATE.REF.imgregistry.MapButton_SitesShopping_1.srcs = "MapButton_Roads_1"
            STATE.REF.imgregistry.MapButton_SitesTransportation_1.srcs = "MapButton_Roads_1"
                ...
                case "!media": {
                    ...
                    switch (D.LCase(call = args.shift())) {
                        ...
                        case "fix": {
                            switch (D.LCase(call = args.shift())) {
                                case "modes": {
                                    resetModeData(D.LCase(args.join(" ")).includes("all"), false, D.LCase(args.join(" ")).includes("verb"), D.LCase(args.join(" ")).includes("confirm"))
                                    break
                                }
                                case "layers": {
                                    setActiveLayers()
                            ...
                            switch (v) {
                                ...
                                case "@@curSrc@@": {
                                    ...
                                    if (!`${mediaObj.get("imgsrc")}`.includes("http")) {
                                        ...
                                        errorLines.push(`<span style="color: darkred; background-color: rgba(100,0,0,0.2);"><b>${objKey}</b>: Object imgsrc set wrong (${D.JSL(mediaObj.get("imgsrc"))}). Setting to top srcRef (${topSrc})</span>`)
                                    }
    ...
    // #region IMG OBJECT & AREA SETTERS: Registering & Manipulating Img Objects
        ...
        startProgressBar = (duration, numTicks, callback) => {
            const funcID = ONSTACK(),
                timeline = TimeTracker.GetRandomTimeline(duration || 20, numTicks || 37),
                tickTimeline = () => {
                    const fID = ONSTACK()
                    if (timeline.length) {
                    ...
                    if (srcURL !== realURL)
                        if (VAL({string: srcURL}) && srcURL.startsWith("http")) {
                            ...
                            if (VAL({string: realSrc}) && realSrc in imgData.srcs) {
                                ...
                                reportStrings.push(`Image source URL doesn't match registry source (= ${D.JS(imgData.curSrc)}) AND couldn't find a valid Src. <span style="color: red; background-color: darkred;"> <b>MANUAL FIX REQUIRED!</b> </span>`)
                            }
                        }
    ...
    // #region SOUND OBJECT GETTERS: Track Object, Playlist Object, Data Retrieval
        ...
        getTrackKey = (soundRef, isGettingAllTracks = false) => {
            ...
            if (isTrack(soundRef)) {
                ...
                if (isGettingAllTracks)
                    return OFFSTACK(funcID) && playlistData.map(x => getSoundKey(x))
                else
                    return OFFSTACK(funcID) && getSoundKey(playlistData.currentTrack[0] || playlistData.trackids[0])
            }
            return OFFSTACK(funcID) && false
        },
        ...
        getTrackObj = (soundRef) => {
            ...
            if (VAL({string: soundKey})) {
                if (soundKey in REGISTRY.TRACKS)
                    return OFFSTACK(funcID) && getObj("jukeboxtrack", REGISTRY.TRACKS[soundKey].id)
                if (soundKey in REGISTRY.PLAYLISTS)
                    return OFFSTACK(funcID) && getObj("jukeboxtrack", REGISTRY.PLAYLISTS[soundKey].currentTrack[0])                
            }
            return OFFSTACK(funcID) && false
        },
        ...
        getScore = (mode, scoreType) => {
            ...
            if (scoreType && scoreList.includes(scoreType))
                return OFFSTACK(funcID) && {[scoreType]: C.SOUNDVOLUME[scoreType] || C.SOUNDVOLUME.defaults.score}
            else
                return OFFSTACK(funcID) && {[scoreList[0]]: C.SOUNDVOLUME[scoreList[0]] || C.SOUNDVOLUME.defaults.score}
        },
    ...
    // #region SOUND OBJECT SETTERS: Registering & Manipulating Music & Sound Effects 
        ...
        updateSounds = (isHardReset = false) => {
            ...
            switch (Session.Mode) {
                case "Active":
                    Object.assign(soundsToPlay, D.KeyMapObj(getWeatherSounds(TimeTracker.WeatherCode), null, (v, k) => {
                        let volume = v[0]
                        if (!Session.IsOutside)
                            if (VAL({number: v[1]})) // A second volume term in the sound configuration is specifically for indoor volume.
                                [,volume] = v
                        ...
                        else if (getWeatherSounds(TimeTracker.WeatherCode).RainLight || getWeatherSounds(TimeTracker.WeatherCode).RainHeavy)
                ...
                case "Spotlight":
                case "Daylighter":
                    Object.assign(soundsToPlay, D.KeyMapObj(getLocationSounds(), null, (v, k) => {
                        let volume = v[0]
                        if (!Session.IsOutside)
                        ...
                        else if (Session.Mode === "Active" && (getWeatherSounds(TimeTracker.WeatherCode).RainLight || getWeatherSounds(TimeTracker.WeatherCode).RainHeavy))
                            volume *= C.SOUNDVOLUME.rainMult[k] || C.SOUNDVOLUME.rainMult.defaults.location || C.SOUNDVOLUME.rainMult.defaults.base
                        return volume
                    }))
                    // falls through
                default: {
                    const scoreCat = TimeTracker.WeatherCode.startsWith("t") && STATE.REF.soundScore === "ScoreMain" && Session.IsOutside && "ScoreIntense" || STATE.REF.soundScore || "ScoreMain"
                    Object.assign(soundsToPlay, D.KeyMapObj(getScore(Session.Mode, scoreCat), null, (v, k) => {
                        let volume = v[0]
                        if (!Session.IsOutside)
