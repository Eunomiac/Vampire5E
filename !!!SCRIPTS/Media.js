...
const Media = (() => {
                            ...
                            case "autopos": {
                                ...
                                if (!["confirm", "skip"].includes(args[0])) {
                                    ...
                                    state.VAMPIRE.Session.locationPointer[STATE.REF.siteNamesOnDeck[0]] = {
                                        pointerPos: {
                                            left: D.Int(mapIndObj.get("left")),
                                            top: D.Int(mapIndObj.get("top"))
                                        }
                                    };
                                    D.Alert(
    ...
    const setZIndices = () => {
        ...
        const [allImgDatas, allAnimDatas, allTextDatas] = [
        ...
        for (const mediaData of Object.values(REGISTRY.ANIM)) {
            ...
            if (assetObj) {
                ASSETREF[assetObj.id] = {
                    ...
                    state: assetObj
                        .get("imgsrc")
                        .replace(/[a-z]*\.(gif|webm|mp4)/gu, "thumb.$1")
                        .replace(new RegExp(C.IMGPREFIX.replace("/", "/"), "g"), ""),
                    isActive: assetObj.get("layer") !== "walls",
                    activeLayer: mediaData.activeLayer,
                    wasModeUpdated: Boolean(mediaData.wasModeUpdated),
        ...
        for (const mediaData of Object.values(REGISTRY.TOKEN)) {
            ...
            if (charObj) {
                ASSETREF[charObj.id] = {
                    ...
                    charID: charObj.id,
                    homePos: D.Clone(
                        _.pick(REGISTRY.AREA[VAL({pc: charObj}) ? `${D.GetCharData(charObj).quadrant}Token` : "SiteCenter"], "top", "left")
                    ),
                    lastPos: D.Clone(
                        _.pick(REGISTRY.AREA[VAL({pc: charObj}) ? `${D.GetCharData(charObj).quadrant}Token` : "SiteCenter"], "top", "left")
                    ),
