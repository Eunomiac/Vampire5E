...
const Media = (() => {
                    ...
                    case "set": {
                        switch (D.LCase(call = args.shift())) {
                            case "mode": {
                                ...
                                // D.Alert(`Sending to ParseToObj: ${args.join(" ")} --> ${D.JS(D.ParseToObj(args.join(" ")))}`)
                                const params = D.ParseToObj(paramArgs.join(" "));
                    ...
                    case "add": {
                        ...
                        switch (D.LCase(call = args.shift())) {
                            ...
                            case "random": case "randomsrc": case "tokensrc": case "tokensource": {
                                ...
                                // srcName = args.join(" ")
                                DB({objects, char: charObj.get("name"), graphicObjs: Listener.GetObjects(objects, "graphic"), tokenSrcObjs, numTokenSrcObjs: tokenSrcObjs.length}, "addTokenSrc");
                        ...
                        switch (D.LCase(call = args.shift())) {
                            ...
                            case "autogeneric": {
                                if (imgSrcAutoGeneric && !args[0]) {
                                    ...
                                    D.Alert(`Automatic registration of generic token images toggled ON.<br><br>Token Category: <b>${D.UCase(imgSrcAutoGeneric)}</b>`, "!img toggle autogeneric"); 
                                }
                                break;
                            }
                            case "autotoken": {
    ...
    // #region ~ VARIABLE DECLARATIONS & JSON DATA
    let [imgRecord, imgResize, imgSrcAutoReg, imgSrcAutoToken, imgSrcAutoGeneric, imgSrcAddingProfilePic] = [false, false, false, false, false, false],
    ...
    const REGISTRY = {
        ...
        get GRAPHIC() { return Object.assign({},
                                             STATE.REF.animregistry,
                                             STATE.REF.imgregistry);
        },
        get ALL() { return Object.assign({}, 
                                         STATE.REF.animregistry, 
                                         STATE.REF.soundregistry, 
    ...
    const getTokenObjs = (charRef, layerFilter = false) => {
        ...
        const allTokenObjs = (findObjs(layerFilter && {_pageid: mainPageID, _type: "graphic", _subtype: "token", layer: layerFilter} || {_pageid: mainPageID, _type: "graphic", _subtype: "token"}) || []).filter(x => isCharToken(x));
    ...
    const addTokenSrc = (tokenSrcRef, charRef, srcName = false) => {
        ...
        const tokenSrc = (VAL({string: tokenSrcRef}) && tokenSrcRef.includes(".png") ? tokenSrcRef || "" : (getImgObj(tokenSrcRef) || {get: () => ""}).get("imgsrc")).replace(/[^\/]*\.png/gu, "thumb.png");
    ...
    const checkDragPads = () => {
        ...
        const mediaDragPadIDs = D.KeyMapObj(_.omit(REGISTRY.IMG, (v) => !v.padID && !v.partnerID), (k, v) => v.id, (v) => [v.padID, v.partnerID]);
    ...
    const clearUnregImgs = (isKilling = false, isQueueing = true) => {
        ...
        const regPadIDs = Object.values(REGISTRY.IMG).filter(x => x.padID).map(x => x.padID);
        const regPartnerIDs = Object.values(REGISTRY.IMG).filter(x => x.partnerID).map(x => x.partnerID);
    ...
    const fixImgObjs = (isQueueing = false) => {
        ...
        const imgPairs = _.zip(imgKeys.map(x => REGISTRY.IMG[x] || REGISTRY.ANIM[x]), imgKeys.map(x => getObj("graphic", (REGISTRY.IMG[x] || REGISTRY.ANIM[x]).id)));
        const reportLines = [];
    ...
    const spreadImgs = (leftImgRef, rightImgRef, midImgRefOrRefs, width, minOverlap = 20, maxOverlap = 40) => {
        ...
        const [leftData, rightData, ...midData] = [leftObj, rightObj, ...midObjs].map(x => isRegImg(x) && getImgData(x) || {id: x.id, name: x.get("name"), left: x.get("left"), width: x.get("width"), leftEdge: x.get("left") - 0.5 * x.get("width"), rightEdge: x.get("left") + 0.5 * x.get("width")});
        if (!VAL({number: width})) {
    ...
    const addPanelText = (text, panelKey = "panel") => {
    ...
    const getSimpleTextWidth = (text, fontFamily, fontSize) => {
        ...
        return TRACEOFF(traceID, _.reduce(sizeArray, (tot, n) => tot + n, 0));
    };
    const getTextWidth = (textRef, text, maxWidth = 0, isSilent = true) => {
        ...
        if (VAL({textObj}, "getTextWidth")) {
            ...
            if (maxWidth !== false && maxW)
                ...
                textLines = textString.split(/\n/g).filter(x => x || x === "" || x === 0).map(x => x.trim());
            if (textLines.length) {
    ...
    const makeText = (hostName, activeLayer, hasShadow, justification, options = {}, funcName = false) => {
        ...
        const objParams = Object.assign({_pageid: D.THISPAGEID,
                                         text: options.text || "",
    ...
    const makeTextShadow = (textRef, params) => {
        ...
        const shadowObj = createObj("text", Object.assign({
    ...
    const fixTextObjs = (isQueueing = false) => {
        ...
        const textPairs = _.zip(textKeys.map(x => REGISTRY.TEXT[x]), textKeys.map(x => getObj("text", REGISTRY.TEXT[x].id)));
        const reportLines = [];
        ...
        for (const mediaData of Object.values(REGISTRY.IMG)) {
            ...
            if (assetObj) {
                ...
                ASSETREF[assetObj.id] = {
                    ...
                    name: assetName,
                    type: "image",
                    page: C.PAGES.GAME === assetObj.get("_pageid") && "GAME" ||
        ...
        for (const mediaData of Object.values(REGISTRY.ANIM)) {
            ...
            if (assetObj) {
                ASSETREF[assetObj.id] = {
                    ...
                    height: D.Int(assetObj.get("height")),
                    width: D.Int(assetObj.get("width")),
                    state: assetObj.get("imgsrc").replace(/[a-z]*\.(gif|webm|mp4)/gu, "thumb.$1").replace(new RegExp(C.IMGPREFIX.replace("/", "\/"), "g"), ""),
                    isActive: assetObj.get("layer") !== "walls",
                    activeLayer: mediaData.activeLayer,
                    wasModeUpdated: Boolean(mediaData.wasModeUpdated),
                    modes: D.Clone(mediaData.modes),
                    srcs: false,
                    timeOut: false,
                    minTimeBetween: false, 
                    maxTimeBetween: false,
                    soundEffect: mediaData.soundEffect || false
                };
                if (D.Int(mediaData.timeOut) > 0) {
                    ASSETREF[assetObj.id].timeOut = D.Int(mediaData.timeOut);
                    ASSETREF[assetObj.id].minTimeBetween = D.Int(mediaData.minTimeBetween);
                    ASSETREF[assetObj.id].maxTimeBetween = D.Int(mediaData.maxTimeBetween);
                }
            } else {
        ...
        for (const mediaData of Object.values(REGISTRY.TOKEN)) {
            ...
            if (charObj) {
                ASSETREF[charObj.id] = {
                    ...
                    srcs: null,
                    charID: charObj.id,
                    homePos: D.Clone(_.pick(REGISTRY.AREA[VAL({pc: charObj}) ? 
                        `${D.GetCharData(charObj).quadrant}Token` :
                        "SiteCenter"], "top", "left")),
                    lastPos: D.Clone(_.pick(REGISTRY.AREA[VAL({pc: charObj}) ? 
                        `${D.GetCharData(charObj).quadrant}Token` :
                        "SiteCenter"], "top", "left")),
                    controllers: _.compact([VAL({pc: charObj}) && D.GetPlayerID(charObj), D.GMID()]),
                    isGenericToken: false,
                    isRandomToken: false
                };
                if (D.GetStatVal(charObj.id, "tokencat")) {
                    ASSETREF[charObj.id].height = 73;
                    ASSETREF[charObj.id].srcs = D.GetStatVal(charObj.id, "tokencat");
                    ASSETREF[charObj.id].isGenericToken = true;
                    ASSETREF[charObj.id].isRandomToken = true;
                } else if (mediaData.srcs.randomSrcs) {
                    ASSETREF[charObj.id].srcs = D.Clone(_.uniq(_.compact([mediaData.srcs.base, ...mediaData.srcs.randomSrcs])).map(x => x.replace(/[a-z]*\.(png|jpg|gif)/gu, "thumb.$1").replace(new RegExp(C.IMGPREFIX.replace("/", "\/"), "g"), "")));
                ...
                errorLines.push(`Error finding character '${mediaData.name}'`);
            }            
        }
