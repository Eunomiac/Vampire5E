...
const Media = (() => {
                        ...
                        switch (D.LCase((call = args.shift()))) {
                            ...
                            default: {
                                ...
                                for (const arg of args)
                                    if (arg in REGISTRY.IMG)
                                        delete REGISTRY.IMG[arg];
                                    else if (`${arg}_1` in REGISTRY.IMG)
                                        delete REGISTRY.IMG[`${arg}_1`];
                                break;
                            }
                        }
                        break;
    ...
    const getMediaPos = (mediaRef) => {
        if (isRegistered(mediaRef)) {
            ...
            if (("shiftTop" in allData) || ("pushtop" in allData))
                posData.top += D.Float(allData.shiftTop, 2) + D.Float(allData.pushtop, 2);
            if (("shiftLeft" in allData) || ("pushleft" in allData))
                posData.left += D.Float(allData.shiftLeft, 2) + D.Float(allData.pushleft, 2);
    ...
    const clearUnregImgs = (isKilling = false, isQueueing = true) => {
        ...
        for (const imgObj of unregImgObjs) {
            ...
            if (isKillingAll || (isKillingNonGMLayer && imgLayer !== "gmlayer"))
                imgObj.remove();
        }
        if (returnLines.length)
            if (isQueueing)
    ...
    const scaleImgObjs = (imgRefs, scaleFactor, baseImgDims) => {
        ...
        for (const imgObj of imgObjs) {
            ...
            if (VAL({number: scaleFactor})) {
                ...
                if (imgContainerData)
                    thisScaleFactor = imgContainerData.tokenScale;
            }
            thisScaleFactor = D.Float(thisScaleFactor || 1);
            if (VAL({list: baseImgDims}))
                thisBaseDims = D.Clone(baseImgDims);
            else if (isGenericToken(imgObj))
                thisBaseDims = D.Clone(IMGDIMS.genericToken);
            else if (isCharToken(imgObj))
                thisBaseDims = D.Clone(IMGDIMS.token);
            else
                thisBaseDims = D.Clone(curDims);
            if (VAL({number: thisScaleFactor, list: thisBaseDims})) {
                const newDims = _.mapObject(thisBaseDims, (x) => D.Int(x * D.Float(thisScaleFactor)));
                if (!_.isEqual(curDims, newDims))
                    trySet(imgObj, newDims);
            }
        }
    };
        // #endregion
    ...
    const setText = (textRef, text, isToggling, isForcing = false) => {
        ...
        if (VAL({list: textData}, "setText")) {
            ...
            if (VAL({textObj}, ["setText", `textRef: ${D.JS(textRef)}, text: ${D.JS(text)}`])) {
                ...
                if (setParams.left === textObj.get("left"))
                    delete setParams.left;
                if (setParams.top === textObj.get("top"))
                    delete setParams.top;
    ...
    const clearUnregText = (isKilling = false, isKillingPlayerText = false) => {
        ...
        for (const textObj of unregTextObjs) {
            ...
            if (textObj.get("pageid") !== C.PAGES.GAME
                    || (!isKillingAll && textLayer === "gmlayer"))
                continue;
            returnLines.push(
            ...
            if (isKillingAll || (isKillingNonGMLayer && textLayer === "gmlayer"))
                textObj.remove();
        }
        if (returnLines.length)
