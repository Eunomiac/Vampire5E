...
const Media = (() => {
    ...
    const onGraphicChange = (imgObj, prevData) => {
        ...
        if (STATE.REF.areImgsBound && isObjActive(imgObj)) {
            ...
            if (imgKey in STATE.REF.boundImages) {
                ...
                for (const boundImgKey of STATE.REF.boundImages[imgKey]) {                             
                    DB({boundImgKey, isActive: isObjActive(boundImgKey)}, "onGraphicChange");
    ...
    const combineTokenSrc = (charRef, srcName = "base") => {
        ...
        if (VAL({tokenObj, string: srcName}) && isRegToken(tokenObj)) {
            ...
            if (srcName === "base")
                srcName = _.uniq([...splitTokenSrcs, ...newTokenSrcs]).find((x) => x in STATE.REF.tokenAuras) || "base";
            if (srcName in STATE.REF.tokenAuras) {
                DB({srcName, auraName: STATE.REF.tokenAuras[srcName], isActive: newTokenSrcs.includes(D.Capitalize(srcName)), isRegAnim: isRegAnim(STATE.REF.tokenAuras[srcName])}, "combineTokenSrc");
                setImgData(STATE.REF.tokenAuras[srcName], {top: tokenObj.get("top"), left: tokenObj.get("left")}, true);
                toggleImg(STATE.REF.tokenAuras[srcName], newTokenSrcs.includes(D.Capitalize(srcName)));
            }
        }
        TRACEOFF(traceID);
    };
