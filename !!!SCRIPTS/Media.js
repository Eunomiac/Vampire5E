...
const Media = (() => {
                    ...
                    switch (D.LCase(call = args.shift())) {
                        case "get": {
                            switch (D.LCase(call = args.shift())) {
                                ...
                                case "volume": {
                                    D.Alert(`<h3>Current Volume Settings</h3>${D.JS(STATE.REF.VOLUME)}`, "!sound get volume")
                                    break
                                }
                                // no default
                            }
                        ...
                        case "set": {
                            switch (D.LCase(call = args.shift())) {
                                case "mastervolume": {
                                    const masterVolumeMult = D.Float(args.shift()),
                                        delveFunc = (ref) => {
                                    ...
                                    if (VAL({number: masterVolumeMult})) {
                                        delveFunc(STATE.REF.VOLUME)
                                        D.Alert(`<h3>New Volume Levels:</h3>${D.JS(STATE.REF.VOLUME)}`, "!sound set mastervolume")
                                        updateSounds(true)
                                    } else {
                                        D.Alert("Must provide a number to multiply sound volumes by!", "!sound set mastervolume")
                                    }
                                    break
                                }
                                case "volume": {
                                    switch (D.LCase(call = args.shift())) {
                                        ...
                                        case "default": {
                                            ...
                                            if (VAL({number: args[0]}) && soundRef in STATE.REF.VOLUME.defaults) {
                                                STATE.REF.VOLUME.defaults[soundRef] = [D.Int(args.shift())]
                                                D.Alert(`<h3>New Volume Levels:</h3>${D.JS(STATE.REF.VOLUME.defaults)}`, "!sound set volume default")
                                            } else {
                                                D.Alert(`Invalid Default. Must be one of: ${D.JS(Object.keys(STATE.REF.VOLUME.defaults))}<br>AND must provide number to change volume to!`, "!sound set volume default [defaultName] [volume]")
                                            } 
                                            break
                                        }
                                        default: {
                                            if (VAL({number: args[0]})) {
                                                STATE.REF.VOLUME[call] = [D.Int(args.shift())]
                                                D.Alert(`Volume of ${call} changed to: ${D.JS(STATE.REF.VOLUME[call])}`, "!sound set volume")
                                            } else {
                                                D.Alert("Must provide a number to set sound volume to!", "!sound set volume [soundName] [volume]")
                                            }
                                            break
                                        }
                                    }
    ...
    // #region SOUND OBJECT GETTERS: Track Object, Playlist Object, Data Retrieval
        ...
        getScore = (mode, scoreType) => {
            ...
            if (scoreType && scoreList.includes(scoreType))
                return OFFSTACK(funcID) && {[scoreType]: STATE.REF.VOLUME[scoreType] || STATE.REF.VOLUME.defaults.score}
            else
                return OFFSTACK(funcID) && {[scoreList[0]]: STATE.REF.VOLUME[scoreList[0]] || STATE.REF.VOLUME.defaults.score}
        },
        getWeatherSounds = (weatherCode) => {
            ...
            if (["w", "p", "t"].includes(weatherCode.charAt(0))) { // Drizzle, Downpour, Thunderstorm
                ...
                weatherSounds[rainSound] = STATE.REF.VOLUME[rainSound] || STATE.REF.VOLUME.defaults.weather || STATE.REF.VOLUME.defaults.base
            }
            ...
            switch (weatherCode.charAt(4)) {            
                case "b": // Breezy / Biting Wind
                    weatherSounds[`${windPrefix}Low`] = STATE.REF.VOLUME[`${windPrefix}Low`] || STATE.REF.VOLUME.defaults.weather || STATE.REF.VOLUME.defaults.base
                    break
                case "w": // Blustery / High Winds
                    weatherSounds[`${windPrefix}Med`] = STATE.REF.VOLUME[`${windPrefix}Med`] || STATE.REF.VOLUME.defaults.weather || STATE.REF.VOLUME.defaults.base
                    break
                case "g": // High Winds / Driving Winds
                    weatherSounds[`${windPrefix}Med`] = STATE.REF.VOLUME[`${windPrefix}Med`] || STATE.REF.VOLUME.defaults.weather || STATE.REF.VOLUME.defaults.base
                    break
                case "h": // Howling Winds / Howling Winds
                    weatherSounds[`${windPrefix}Max`] = STATE.REF.VOLUME[`${windPrefix}Max`] || STATE.REF.VOLUME.defaults.weather || STATE.REF.VOLUME.defaults.base
                    break
        ...
        getLocationSounds = () => {
            ...
            if (Object.values(locSounds).includes("(TOTALSILENCE)"))
                ...
                finalLocSound = {[locSounds.Site]: STATE.REF.VOLUME[locSounds.Site] || STATE.REF.VOLUME.defaults.location || STATE.REF.VOLUME.defaults.base}
            else if (locSounds.District && locSounds.District !== "(NONE)" && Session.IsOutside)
                finalLocSound = {[locSounds.District]: STATE.REF.VOLUME[locSounds.District] || STATE.REF.VOLUME.defaults.location || STATE.REF.VOLUME.defaults.base}
    ...
    // #region SOUND OBJECT SETTERS: Registering & Manipulating Music & Sound Effects 
        ...
        updateSounds = (isHardReset = false) => {
            ...
            switch (Session.Mode) {
                    ...
                    Object.assign(soundsToPlay, D.KeyMapObj(getWeatherSounds(TimeTracker.WeatherCode), null, (v, k) => {
                        ...
                        if (!Session.IsOutside)
                            if (VAL({number: v[1]})) // A second volume term in the sound configuration is specifically for indoor volume.
                                [,volume] = v
                            else if (VAL({number: STATE.REF.VOLUME.indoorMult[k]}))
                                volume *= STATE.REF.VOLUME.indoorMult[k]
                            else if (VAL({number: STATE.REF.VOLUME.indoorMult.defaults.weather}))
                                volume *= STATE.REF.VOLUME.indoorMult.defaults.weather
                            else
                                volume *= STATE.REF.VOLUME.indoorMult.defaults.base
                        else if (getWeatherSounds(TimeTracker.WeatherCode).RainLight || getWeatherSounds(TimeTracker.WeatherCode).RainHeavy)
                            if (VAL({number: STATE.REF.VOLUME.rainMult[k]}))
                                volume *= STATE.REF.VOLUME.rainMult[k]
                            else if (VAL({number: STATE.REF.VOLUME.rainMult.defaults.weather}))
                                volume *= STATE.REF.VOLUME.rainMult.defaults.weather
                            else
                                volume *= STATE.REF.VOLUME.rainMult.defaults.base
                        return volume
                    }))
                    // falls through
                case "Downtime":
                ...
                case "Daylighter":
                    Object.assign(soundsToPlay, D.KeyMapObj(getLocationSounds(), null, (v, k) => {
                        let volume = v[0]
                        if (!Session.IsOutside)
                            volume = v[1] || v[0] * (STATE.REF.VOLUME.indoorMult[k] || STATE.REF.VOLUME.indoorMult.defaults.location || STATE.REF.VOLUME.indoorMult.defaults.base)
                            ...
                            volume *= STATE.REF.VOLUME.rainMult[k] || STATE.REF.VOLUME.rainMult.defaults.location || STATE.REF.VOLUME.rainMult.defaults.base
                        return volume
                    }))
                    // falls through
                default: {
                    ...
                    Object.assign(soundsToPlay, D.KeyMapObj(getScore(Session.Mode, scoreCat), null, (v, k) => {
                        let volume = v[0]
                        if (!Session.IsOutside)
                            volume = v[1] || v[0] * (STATE.REF.VOLUME.indoorMult[k] || STATE.REF.VOLUME.indoorMult.defaults.score || STATE.REF.VOLUME.indoorMult.defaults.base)
                        else if (Session.Mode === "Active" && (getWeatherSounds(TimeTracker.WeatherCode).RainLight || getWeatherSounds(TimeTracker.WeatherCode).RainHeavy))
                            volume *= STATE.REF.VOLUME.rainMult[k] || STATE.REF.VOLUME.rainMult.defaults.score || STATE.REF.VOLUME.rainMult.defaults.base
                        return volume
                    }))
                    break
                }
            ...
            if (Object.keys(soundsToPlay).includes("TOTALSILENCE")) {
                ...
                for (const thunderSoundRef of ["Thunder", ..._.intersection(Roll20AM.GetPlaylistTrackNames("Thunder"), Object.keys(STATE.REF.VOLUME))]) {
                    let thunderVol = STATE.REF.VOLUME[thunderSoundRef] || thunderSoundRef === "Thunder" && (STATE.REF.VOLUME.defaults.effect || STATE.REF.VOLUME.defaults.base)
                    if (thunderVol) {
                        if (Session.IsOutside)
                            thunderVol = VAL({number: thunderVol[0]}) ? thunderVol[0] : thunderVol
                        else
                            thunderVol = VAL({number: thunderVol[1]}) ? thunderVol[1] : thunderVol[0] * (STATE.REF.VOLUME.indoorMult[thunderSoundRef] || STATE.REF.VOLUME.indoorMult.defaults.effect || STATE.REF.VOLUME.indoorMult.defaults.base)
    ...
    return {
        ...
        GetImgSrc: getImgSrc,
        GetTokens: getTokenObjs, GetTokenData: getTokenData,
