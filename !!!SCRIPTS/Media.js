...
const Media = (() => {
    ...
    const initialize = () => {
        ...
        setImgData("RollerFrame_OppOutcome", {height: 30, width: 90}, true);
        ...
        STATE.REF.layoutregistry = STATE.REF.layoutregistry || {};
            ...
            case "!media": {
                ...
                switch (D.LCase((call = args.shift()))) {
                    ...
                    case "test": {
                        switch (D.LCase((call = args.shift()))) {
                            case "layoutpos": {
                                const layoutName = args.shift();
                                const layoutData = REGISTRY.LAYOUT[layoutName];
                                if (VAL({list: layoutData})) {
                                    REGISTRY.LAYOUT[layoutName].deltaPosData = {};
                                    [...layoutData.preReqMediaRefs, ...Object.keys(_.omit(layoutData, "preReqMediaRefs", "deltaPosData", "curPosData"))].forEach((x) => { toggle(x, true) });
                                    _.range(4, 31).forEach((x) => toggle(`RollerDie_Opp_${x}`, false));
                                    const layoutPos = getLayoutPos(layoutName) || {};                                    
                                    D.Show(getLayoutPos(layoutName) || {});
                                    for (const [key, posData] of Object.entries(layoutPos))
                                        if (isRegImg(key)) {
                                            setImgData(key, _.omit(posData, "leftEdge", "rightEdge", "topEdge", "bottomEdge"), true);
                                        } else {
                                            const textPosData = _.omit(posData, "leftEdge", "rightEdge", "topEdge", "bottomEdge");
                                            const textData = getTextData(key);
                                            if (textData.justification === "left") {
                                                textPosData.left = posData.leftEdge;
                                            } else if (textData.justification === "right") {
                                                textPosData.left = posData.rightEdge;
                                            }
                                            setTextData(key, textPosData);
                                        }
                                } else {
                                    D.Flag("Must Supply a Valid Layout Name!");
                                }
                                break;
                            }
                            case "layout": {
                                const layoutName = args.shift();
                                if (VAL({string: layoutName})) {
                                    const layoutData = Object.assign({
                                        RollerFrame_OppNameLeft: {
                                            topEdge: {key: "RollerFrame_Left", pos: "bottomEdge"},
                                            leftEdge: {key: "RollerFrame_Left", pos: "leftEdge", shift: 20}
                                        },
                                        oppRollerName: {
                                    ...
                                    if (args.join(" ").includes("apply")) {                                    
                                        regLayout(layoutName, layoutData);
                                    } else {
                                        D.Show(layoutData);
                                        D.Flag("'!media test layout apply' TO APPLY");
                                    }
                                } else {
                                    D.Flag("Must Supply Layout Name!");
                                }
                                break;
                            }
                            // no default
    ...
    const REGISTRY = {
        ...
        get LAYOUT() {
            return STATE.REF.layoutregistry;
        },
        get GRAPHIC() {
    ...
    // #region LAYOUTS: MEDIA OBJECT SETS POSITIONED BY REFERENCE
    const regLayout = (layoutKey, layoutData) => {
        /* LAYOUT DATA:
            const layoutData = {
                preReqMediaRefs: ["RollerFrame_Diff", "RollerFrame_Left"},
                curPosData: {RollerFrame_OppNameLeft: {left, top, height, width, leftEdge, rightEdge, topEdge, bottomEdge}},
                deltaPosData: {RollerFrame_OppNameLeft: {left}},
                RollerFrame_OppNameLeft: {
                    topEdge: {RollerFrame_Left: "bottomEdge"},
                    leftEdge: {RollerFrame_Diff: "rightEdge", shift: 20}
                },
                oppRollerName: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppNameLeft: "leftEdge", shift: 20}
                },
                RollerFrame_OppNameMid: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppNameLeft: "rightEdge"},
                    width: {oppRollerName: "width"}
                },
                RollerFrame_OppNameRight: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppNameMid: "rightEdge"}
                },
                RollerFrame_OppDicePool: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    left: {RollerFrame_OppNameRight: "rightEdge"}
                },
                RollerFrame_OppMainLeft: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppNameRight: "rightEdge"}
                },
                oppRollerMain: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppMainLeft: "leftEdge", shift: 20}
                },
                RollerFrame_OppMainMid: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppMainLeft: "rightEdge"},
                    width: {oppRollerMain: "width"}
                },
                RollerFrame_OppMainRight: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppMainMid: "rightEdge"}
                },
                RollerFrame_OppDiceBoxLeft: {
                    topEdge: {RollerFrame_OppNameLeft: "topEdge"},
                    leftEdge: {RollerFrame_OppMainRight: "rightEdge"}
                },
                RollerFrame_OppDiceBoxMid: {
                    top: {RollerFrame_OppDiceBoxLeft: "top"},
                    leftEdge: {RollerFrame_OppDiceBoxLeft: "rightEdge"},
                    rightEdge: {RollerDie_Opp_30: "rightEdge", shift: 20}
                },
                RollerFrame_OppDiceBoxRight: {
                    top: {RollerFrame_OppDiceBoxLeft: "top"},
                    leftEdge: {RollerFrame_OppDiceBoxMid: "rightEdge"}
                },
                RollerFrame_OppMarginOpp: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppDiceBoxRight: "rightEdge", shift: 10}
                },
                oppMarginOpp: {
                    top: {RollerFrame_OppMarginOpp: "top"},
                    left: {RollerFrame_OppMarginOpp: "left"}
                },
                RollerFrame_OppCompVS: {
                    top: {RollerFrame_OppMarginOpp: "top"},
                    leftEdge: {RollerFrame_OppMarginOpp: "rightEdge", shift: -10}
                },
                RollerFrame_OppMarginMain: {
                    top: {RollerFrame_OppNameLeft: "top"},
                    leftEdge: {RollerFrame_OppCompVS: "rightEdge", shift: -10}
                },
                oppMarginMain: {
                    top: {RollerFrame_OppMarginMain: "top"},
                    left: {RollerFrame_OppMarginMain: "left"}
                }
            }
        */
        const errorData = {};
        const regLayoutData = {};
        const preReqMediaRefs = [];
        const validPosRefs = ["left", "top", "height", "width", "leftEdge", "rightEdge", "topEdge", "bottomEdge"];
        for (const [mediaKey, posData] of Object.entries(layoutData))
            if (isRegistered(mediaKey)
                && !Object.values(_.pick(getMediaData(mediaKey), (v, k) => ["shiftLeft", "shiftTop", "pushleft", "pushtop"].includes(k) && v)).length
                && _.every(posData, (v, k) => validPosRefs.includes(k) && isRegistered(v.key) && validPosRefs.includes(v.pos) && (!("shift" in v) || VAL({number: v.shift})))) {
                Object.values(posData).forEach((x) => {
                    if (!(x.key in layoutData))
                        preReqMediaRefs.push(x.key);
                });
                regLayoutData[mediaKey] = posData;
            } else {
                errorData[mediaKey] = `<b><u>Error Registering '${mediaKey}'</u></b><br>${D.JS({
                    isRegistered: isRegistered(mediaKey),
                    allValidPosRefs: _.every(posData, (v, k) => validPosRefs.includes(k)),
                    allValidPosData: _.every(posData, (v, k) => _.every(v, (vv, kk) => (kk === "shift" && VAL({number: vv})) || (isRegistered(kk) && validPosRefs.includes(vv))))
                })}`;
            }
        if (_.isEmpty(errorData)) {
            REGISTRY.LAYOUT[layoutKey] = Object.assign({
                curPosData: {},
                deltaPosData: {},
                preReqMediaRefs: _.uniq(preReqMediaRefs)
            }, regLayoutData);
            D.Alert(`<h3>Layout '${layoutKey}' Registered</h3><br>${D.JS(REGISTRY.LAYOUT[layoutKey])}`, "regLayout");
        } else {
            D.Alert(`<h3 style="color: red;">Error Registering Layout '${D.UCase(layoutKey)}'</h3><br>${D.JS(errorData)}`, "regLayout");
        }
    };
    const getPosData = (layoutKey, mediaKey, curMediaPosData = {}) => {
        
        /* const layoutData = {
            curPosData: {},
            deltaPosData: {},
            preReqMediaRefs: ["RollerFrame_Left", "RollerFrame_Diff"],
            RollerFrame_OppNameLeft: {
              topEdge: { key: "RollerFrame_Left", pos: "bottomEdge" },
              leftEdge: { key: "RollerFrame_Diff", pos: "rightEdge", shift: 20 }
            },
            oppRollerName: {
              top: { key: "RollerFrame_OppNameLeft", pos: "top" },
              leftEdge: { key: "RollerFrame_OppNameLeft", pos: "leftEdge", shift: 20 }
            ...
            RollerFrame_OppNameMid: {
              top: { key: "RollerFrame_OppNameLeft", pos: "top" },
              leftEdge: { key: "RollerFrame_OppNameLeft", pos: "rightEdge" },
              width: { key: "oppRollerName", pos: "width" }
            },
            RollerFrame_OppNameRight: {
              top: { key: "RollerFrame_OppNameLeft", pos: "top" },
              leftEdge: { key: "RollerFrame_OppNameMid", pos: "rightEdge" }
            },
            RollerFrame_OppDicePool: {
              top: { key: "RollerFrame_OppNameLeft", pos: "top" },
              left: { key: "RollerFrame_OppNameRight", pos: "rightEdge" }
            ...
            RollerFrame_OppMainMid: {
              top: { key: "RollerFrame_OppNameLeft", pos: "top" },
              leftEdge: { key: "RollerFrame_OppMainLeft", pos: "rightEdge" },
              width: { key: "oppMainRoll", pos: "width" }
            },
            RollerFrame_OppMainRight: {
              top: { key: "RollerFrame_OppNameLeft", pos: "top" },
              leftEdge: { key: "RollerFrame_OppMainMid", pos: "rightEdge" }
            },
            RollerFrame_OppDiceBoxLeft: {
              topEdge: { key: "RollerFrame_OppNameLeft", pos: "topEdge" },
              leftEdge: { key: "RollerFrame_OppMainRight", pos: "rightEdge" }
            },
            RollerDie_Opp_1: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerFrame_OppDiceBoxLeft", pos: "leftEdge", shift: 10 }
            ...
            RollerDie_Opp_2: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_1", pos: "left", shift: 5 }
            },
            RollerDie_Opp_3: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_2", pos: "left", shift: 5 }
            },
            RollerDie_Opp_4: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_3", pos: "left", shift: 5 }
            },
            RollerDie_Opp_5: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_4", pos: "left", shift: 5 }
            },
            RollerDie_Opp_6: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_5", pos: "left", shift: 5 }
            },
            RollerDie_Opp_7: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_6", pos: "left", shift: 5 }
            },
            RollerDie_Opp_8: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_7", pos: "left", shift: 5 }
            },
            RollerDie_Opp_9: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_8", pos: "left", shift: 5 }
            },
            RollerDie_Opp_10: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_9", pos: "left", shift: 5 }
            },
            RollerDie_Opp_11: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_10", pos: "left", shift: 5 }
            },
            RollerDie_Opp_12: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_11", pos: "left", shift: 5 }
            },
            RollerDie_Opp_13: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_12", pos: "left", shift: 5 }
            },
            RollerDie_Opp_14: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_13", pos: "left", shift: 5 }
            },
            RollerDie_Opp_15: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_14", pos: "left", shift: 5 }
            },
            RollerDie_Opp_16: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_15", pos: "left", shift: 5 }
            },
            RollerDie_Opp_17: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_16", pos: "left", shift: 5 }
            },
            RollerDie_Opp_18: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_17", pos: "left", shift: 5 }
            },
            RollerDie_Opp_19: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_18", pos: "left", shift: 5 }
            },
            RollerDie_Opp_20: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_19", pos: "left", shift: 5 }
            },
            RollerDie_Opp_21: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_20", pos: "left", shift: 5 }
            },
            RollerDie_Opp_22: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_21", pos: "left", shift: 5 }
            },
            RollerDie_Opp_23: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_22", pos: "left", shift: 5 }
            },
            RollerDie_Opp_24: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_23", pos: "left", shift: 5 }
            },
            RollerDie_Opp_25: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_24", pos: "left", shift: 5 }
            },
            RollerDie_Opp_26: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_25", pos: "left", shift: 5 }
            },
            RollerDie_Opp_27: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_26", pos: "left", shift: 5 }
            },
            RollerDie_Opp_28: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_27", pos: "left", shift: 5 }
            },
            RollerDie_Opp_29: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_28", pos: "left", shift: 5 }
            },
            RollerDie_Opp_30: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              left: { key: "RollerDie_Opp_29", pos: "left", shift: 5 }
            },
            RollerFrame_OppDiceBoxMid: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              leftEdge: { key: "RollerFrame_OppDiceBoxLeft", pos: "rightEdge" },
              rightEdge: { key: "RollerDie_Opp_30", pos: "rightEdge", shift: 20 }
            },
            RollerFrame_OppDiceBoxRight: {
              top: { key: "RollerFrame_OppDiceBoxLeft", pos: "top" },
              leftEdge: { key: "RollerFrame_OppDiceBoxMid", pos: "rightEdge" }
            },
            RollerFrame_OppMarginOpp: {
              top: { key: "RollerFrame_OppNameLeft", pos: "top" },
              leftEdge: {
                key: "RollerFrame_OppDiceBoxRight",
                pos: "rightEdge",
                shift: 10
              }
            },
        ...
        const posData = {};
        const errorLines = [];
        if (layoutKey in REGISTRY.LAYOUT) {
            const fullLayoutData = REGISTRY.LAYOUT[layoutKey];
            if (mediaKey in curMediaPosData)
                return curMediaPosData[mediaKey];
            if (fullLayoutData.preReqMediaRefs.includes(mediaKey))
                return getMediaPos(mediaKey);
            if (isRegistered(mediaKey) && (mediaKey in fullLayoutData)) {
                const layoutData = fullLayoutData[mediaKey];

                // GET CURRENT POS DATA:
                const curPosData = Object.assign(getMediaPos(mediaKey), fullLayoutData.curPosData[mediaKey] || {}, fullLayoutData.deltaPosData[mediaKey] || {});
                const layoutPosData = {};
                // If height/width aren't included, assign them UNLESS both edges are present:
                if (!_.has(layoutData, "width") && !(_.has(layoutData, "leftEdge") && _.has(layoutData, "rightEdge")))
                    layoutPosData.width = curPosData.width;
                if (!_.has(layoutData, "height") && !(_.has(layoutData, "topEdge") && _.has(layoutData, "bottomEdge")))
                    layoutPosData.height = curPosData.height;
                // Convert layout references to actual positions:
                try {
                    for (const [pos, data] of Object.entries(layoutData)) {
                        // DB({pos, data}, "getPosData");
                        layoutPosData[pos] = VAL({number: layoutPosData[pos]}) ? layoutPosData[pos] : getPosData(layoutKey, data.key, Object.assign({}, curMediaPosData, {[mediaKey]: curPosData}, {[mediaKey]: layoutPosData}))[data.pos] + (isObjActive(mediaKey) && D.Float(data.shift, 2) || 0);
                        REGISTRY.LAYOUT[layoutKey].deltaPosData[mediaKey] = Object.assign(REGISTRY.LAYOUT[layoutKey].deltaPosData[mediaKey] || {}, curPosData, layoutPosData);
                        DB({layoutData, layoutPosData}, "getPosData");
                        // return;
                    }
                } catch (errObj) {
                    D.Alert(`Pos Data Error (${D.JSL(layoutKey)}, ${D.JSL(mediaKey)}): ${D.JS(layoutPosData)}`, "getPosData");
                    return;
                }

                // return;
                // Assign layoutPosData to curPosData:
                Object.assign(curPosData, layoutPosData);
                const quadCheck = (posNeeds, posHas) => _.every(posNeeds, (x) => !_.has(layoutPosData, x)) && _.every(posHas, (x) => _.has(layoutPosData, x));
                if (quadCheck(["left", "width"], ["leftEdge", "rightEdge"])) {
                    curPosData.width = curPosData.rightEdge - curPosData.leftEdge;
                    curPosData.left = curPosData.leftEdge + 0.5 * curPosData.width;
                } else if (quadCheck(["left", "leftEdge"], ["width", "rightEdge"])) {
                    curPosData.left = curPosData.rightEdge - 0.5 * curPosData.width;
                    curPosData.leftEdge = curPosData.rightEdge - curPosData.width;
                } else if (quadCheck(["left", "rightEdge"], ["width", "leftEdge"])) {
                    curPosData.left = curPosData.leftEdge + 0.5 * curPosData.width;
                    curPosData.rightEdge = curPosData.leftEdge + curPosData.width;
                } else if (quadCheck(["width", "leftEdge"], ["left", "rightEdge"])) {
                    curPosData.width = 2 * (curPosData.rightEdge - curPosData.left);
                    curPosData.leftEdge = curPosData.left - 0.5 * curPosData.width;
                } else if (quadCheck(["width", "rightEdge"], ["left", "leftEdge"])) {
                    curPosData.width = 2 * (curPosData.left - curPosData.leftEdge);
                    curPosData.rightEdge = curPosData.left + 0.5 * curPosData.width;
                } else if (quadCheck(["leftEdge", "rightEdge"], ["left", "width"])) {
                    curPosData.leftEdge = curPosData.left - 0.5 * curPosData.width;
                    curPosData.rightEdge = curPosData.left + 0.5 * curPosData.width;
                } else {
                    errorLines.push(`Unable to Derive Horizontal Position for '${mediaKey}'`);
                }

                if (quadCheck(["top", "height"], ["topEdge", "bottomEdge"])) {
                    curPosData.height = curPosData.bottomEdge - curPosData.topEdge;
                    curPosData.top = curPosData.topEdge + 0.5 * curPosData.height;
                } else if (quadCheck(["top", "topEdge"], ["height", "bottomEdge"])) {
                    curPosData.top = curPosData.bottomEdge - 0.5 * curPosData.height;
                    curPosData.topEdge = curPosData.bottomEdge - curPosData.height;
                } else if (quadCheck(["top", "bottomEdge"], ["height", "topEdge"])) {
                    curPosData.top = curPosData.topEdge + 0.5 * curPosData.height;
                    curPosData.bottomEdge = curPosData.topEdge + curPosData.height;
                } else if (quadCheck(["height", "topEdge"], ["top", "bottomEdge"])) {
                    curPosData.height = 2 * (curPosData.bottomEdge - curPosData.top);
                    curPosData.topEdge = curPosData.top - 0.5 * curPosData.height;
                } else if (quadCheck(["height", "bottomEdge"], ["top", "topEdge"])) {
                    curPosData.height = 2 * (curPosData.top - curPosData.topEdge);
                    curPosData.bottomEdge = curPosData.top + 0.5 * curPosData.height;
                } else if (quadCheck(["topEdge", "bottomEdge"], ["top", "height"])) {
                    curPosData.topEdge = curPosData.top - 0.5 * curPosData.height;
                    curPosData.bottomEdge = curPosData.top + 0.5 * curPosData.height;
                } else {
                    errorLines.push(`Unable to Derive Vertical Position for '${mediaKey}'`);
                }

                if (errorLines.length) {
                    D.Alert(`Error calculating position of '${mediaKey}': ${D.JS(curPosData)}`, "getPosData");
                    return {};
                } else {
                    // If this object is NOT active, set position and edges accordingly:
                    if (!isObjActive(mediaKey)) {
                        curPosData.height = 0;
                        curPosData.width = 0;
                        curPosData.left = curPosData.leftEdge;
                        curPosData.rightEdge = curPosData.leftEdge;
                        curPosData.top = curPosData.topEdge;
                        curPosData.bottomEdge = curPosData.topEdge;
                    }
                    // Add derived data to registry
                    REGISTRY.LAYOUT[layoutKey].deltaPosData[mediaKey] = Object.assign(REGISTRY.LAYOUT[layoutKey].deltaPosData[mediaKey] || {}, curPosData);
    ...
    const getLayoutPos = (layoutKey) => {
        if (layoutKey in REGISTRY.LAYOUT) {
            const layoutPosData = {};
            let count = 0;
            for (const mediaKey of Object.keys(_.omit(REGISTRY.LAYOUT[layoutKey], "preReqMediaRefs", "curPosData", "deltaPosData"))) {
                DB({START: [layoutKey, mediaKey]}, "getPosData");
                count++;
                layoutPosData[mediaKey] = getPosData(layoutKey, mediaKey, layoutPosData);
                /* if (count >= 15)
                    return layoutPosData; */
            }
            return layoutPosData;
        }
        return THROW(`'${layoutKey}' is not a registered layout.`, "getLayoutPos");
    };
    // #endregion
