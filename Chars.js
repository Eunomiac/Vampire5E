var Chars = Chars || (function () {
  ...
  const MVCVALS = [
    ...
    [
      'Pivotal Event',
      ['para', '<span style="font-size: 14px; margin-top: 10px;"><b>Before your Embrace ...</b></span><br>... you fell in love with the wrong person, who dragged you into an exciting world you never knew existed. You freaked out and left them.',
      ...
      ['para', '<span style="font-size: 14px; margin-top: 10px;"><b>You <i>still</i> feel <span style="font-size: 14px; margin-top: 10px;"><u>Endless Regret</u>:</b></span><br>You didn�t comport yourself with any dignity or honesty. You thought you were better than that.',
      ...
      ['para', "<span style=\"font-size: 14px; margin-top: 10px;\"><b>You can't let go because ... </b></span><br> ... it never ended. You just move forward and try to put the past behind you. But when you can't help but look back, it's always there, right at your heels.",
    ...
    [
      'Habits',
      ['paraStart', 'You reinforce or establish your <b>Profession</b> ',
      ...
      ['paraMid', 'with your <b>Speech and Accent</b>, but undermine, subvert or contradict it ',
      ...
      ['paraEnd', 'with your <b>Speech and Accent</b>.',
    ...
    [
      'Signposts',
      ['paraStart', 'When you see or commit an <b>Abuse of Power</b>, ',
      ...
      ['paraEnd', 'you react with <b>Hatred</b> (or) <b>Love:</b> You want to destroy them for existing, or to bask in their glory.',
  ...
  var registerChars = function (chars) {
  ...
  var getChars = function () {
    ...
    _.each(state[D.GAMENAME].Chars, function (v, k) {
      if (k === v.id) { charArray.push(D.GetChar(v.id)) }
    })
    return charArray
  }

  var getAttr = function (charObj, attr) {
    attr = attr.toLowerCase().replace(/\s+/g, '').replace(/ /g, '')
    return filterObjs(function (obj) {
      let name = obj.get('name').toLowerCase().replace(/\s+/g, '').replace(/ /g, '')
      if (obj.get(' type') === 'attribute' &&
                obj.get(' characterid') == charObj.id &&
                name == attr) { return true } else return false
    })[0]
  }
  // #endregion

  // #region Awarding XP
  var awardXP = function (char, award, session, reason) {
    if (!state[D.GAMENAME].Chars[char.id]) { return D.ThrowError('You must register the character first.', 'AWARDXP') }
    let rowID = D.MakeRow(char, 'earnedxp', {
      xp_award: award,
      xp_session: session,
      xp_reason: reason
    })
    if (rowID) { return true } else { return false }
  }
  // #endregion

  // #region Manipulating Stats on Sheet
  var adjustDamage = function (charObj, trait, dtype, amount) {
    var attrList = {}
  ...
  var MVC = function (params) {
    var results = []
    for (var i = 0; i < MVCVALS.length; i++) {
      results.push(bHTML.div.title.start + MVCVALS[i][0] + bHTML.div.title.stop)
      for (var j = 1; j < MVCVALS[i].length; j++) {
        let fType = MVCVALS[i][j][0]
        // D.Log("FTYPE: " + D.JSL(fType));
        let randItem = _.shuffle(MVCVALS[i][j].slice(1))[0]
    ...
    let finalCode = HTML.start + results.join('') + HTML.stop
  ...
  var handleInput = function (msg) {
    ...
    var who = (getObj('player', msg.playerid) || { get: () => 'API' }).get('displayname')
    // D.Log("WHO: " + D.JSL(who));
    let args = msg.content.split(/\s+/)
    switch (args.shift()) {
      case '!rChar':
        if (!playerIsGM(msg.playerid)) { return }
        if (!msg.selected || !msg.selected[0]) { return D.Alert('Select character tokens first!') }
        registerChars(D.GetChars(msg))
        break
      case '!xp': // !xp Cost Session Message, with some character tokens selected.
        if (!playerIsGM(msg.playerid)) { return }
        var chars = D.GetChars(msg)
        if (!chars) { return D.ThrowError('Select one or more character tokens!') }
        let amount = args.shift()
        let session = args.shift()
        _.each(chars, function (char) {
          if (awardXP(char, amount, session, args.join(' '))) { D.Alert(amount + ' XP awarded to ' + D.GetName(char), '!XP') } else { D.ThrowError('FAILED to award ' + JSON.stringify(amount) + ' XP to ' + JSON.stringify(D.GetName(char)), '!XP') }
        })
        break
      case '!dmg':
        if (!playerIsGM(msg.playerid)) { return }
        var chars = D.GetChars(msg)
        if (!chars) { return D.ThrowError('Select one or more character tokens!') }
        let trait = args.shift()
        let dtype = args.shift()
        let dmgAmount = parseInt(args.shift())
        _.each(chars, function (char) {
          if (adjustDamage(char, trait, dtype, dmgAmount)) { D.Alert('Dealt ' + D.JS(dmgAmount) + ' ' + D.JS(dtype) + ' ' + D.JS(trait) + ' damage to ' + D.GetName(char)) } else { D.ThrowError('FAILED to damage ' + D.GetName(char)) }
        })
        break
      case '!getIncap':
        if (!playerIsGM(msg.playerid)) { return }
        let incap
        if (msg.selected && msg.selected[0]) { D.Alert("Incapacitation String for '" + D.GetName(D.GetChar(msg)) + "': <br/><br/>" + D.GetStat(msg, 'incapacitation').get('current') + '<br/><br/>e.g. !setIncap Compulsion (Arrogance):a:-2<br/><br/>OR<br/><br/>!setIncap Compulsion (Arrogance):Strength,Dexterity,Animal Ken,Dominate:-2') } else { D.Alert('Select a character first!') }
        break
      case '!setIncap':
        if (!playerIsGM(msg.playerid)) { return }
        let incapString = args.join(' ')
      ...
      case '!setComp':
        ...
        if (msg.selected && msg.selected[0]) {
          ...
          if (params.length !== 2) { return D.Alert("Must supply a title and a message separated by a pipe '|'", '!setComp title|message') }
      ...
      case '!setDyscrasias':
        ...
        if (msg.selected && msg.selected[0]) {
          let params = args.join(' ').split('|')
          if (params.length !== 2) { return D.Alert("Must supply a title and a message separated by a pipe '|'", '!setDys title|message') }
          setAttrs(D.GetChar(msg).id, {
      ...
      case '!getProj':
        if (!playerIsGM(msg.playerid)) { return }
        if (msg.selected && msg.selected[0]) {
          let attrs = _.filter(findObjs({ _type: 'attribute', _characterid: D.GetChar(msg).id }), a => a.get('name').toLowerCase().includes('repeating_project_'))
      ...
      case '!delProj':
        ...
        if (msg.selected && msg.selected[0]) {
          ...
          attrs = _.filter(findObjs({ _type: 'attribute', _characterid: D.GetChar(msg).id }), a => a.get('name').toLowerCase().includes('repeating_project_'))
