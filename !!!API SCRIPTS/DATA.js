    ...
    */
    ...
    const getSelected = (msg, typeFilter = []) => {
        ...
        getChars = (charRef, isSilent = false) => {
            ...
            if (charObjs.size === 0)
                return isSilent ? false : throwError(`No Characters Found using Search Parameters:<br><br>${jStr(searchParams)} in Character Reference<br><br>${jStr(charRef)}`, "getChars", SCRIPTNAME)
        ...
        makeRepRow = (charRef, secName, attrs) => {
            ...
            _.each(attrs, (v, k) => {
                if (_.isString(prefix + k) && (prefix + k).length > 12) {
                    ...
                    throwError(`Failure at makeRepRow(charRef, ${D.JSC(secName)}, ${D.JSC(attrs)})<br><br>Prefix (${D.JSC(prefix)}) + K (${D.JSC(k)}) is NOT A STRING asd})`, "makeRepRow", SCRIPTNAME)
                }
            })
