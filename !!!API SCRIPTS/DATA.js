...
var D = D || (function () {
    ...
    let VALS = {
        PAGEID: function () { return Campaign().get('playerpageid') },
    ...
    const JSONStringify = function (obj, isLogOnly) {
        try {
            ...
            if (isLogOnly)
                ...
                return JSON.stringify(obj, null, "    ").replace(/\"/g, "'").replace(/ /g, "&nbsp;").replace(/\n/g, "<br/>").replace(/&nbsp;&nbsp;/g, "&nbsp;");
        } catch (err) {
    ...
    const getName = function (value, isShort) {
        ...
        if (name && isShort)
            if (name.includes('"'))
                return name.replace(/.*?["]/i, "").replace(/["].*/i, "").replace(/_/g, " ");
            else
                return str.replace(/.*\s/i, "").replace(/_/g, " ");
        else if (name)
            return name;
    ...
    const getChars = function (value) {
        ...
        if (value.who) {
            ...
            return formatError("Bad Value: '" + JSONStringify(value) + "'", "GETCHARS");
        let charObjs = [];
        _.each(searchParams, function (v) {
            ...
            if (_.isString(v) && getObj("character", v))
                ...
                if (getObj("character", v.get("represents")))
                    theseCharIDs.push(getObj("character", v.get("represents")));
                else
                ...
                theseCharObjs = findObjs({ _type: "character", name: v });
    ...
    const getPlayerID = function (value) {
        ...
        try {
            ...
            if (value.get("_type") === "character")
                playerID = value.get("controlledby").replace("all,", "").replace(",all", "").split(",", 1)[0];
    ...
    const sendChatMessage = function (who, message, title) {
        ...
        if (_.isArray(message))
            _.each(message, function (v) {
                if (_.isObject(v))
                    sendChatMessage(who, v, title);
                else
                    messageString += parseChatLine(v);
            });
        else if (_.isObject(message))
            sendChatMessage(who, message.message || "", message.title || title);
        else
            messageString += parseChatLine(message);
        if (messageString !== "") {
    ...
    const formatAlert = function (msg, title) {
        ...
        sendChatMessage("Storyteller", msg, title ? title : "[ALERT]");
        return false;
    }
