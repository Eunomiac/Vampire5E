...
// #region Constants and Declarations
	...
	const awardXP = (char, award, session, reason) => {
	...
	const adjustDamage = (charObj, trt, dtype, amount) => {
		const attrList = {}

				
		const trait = trt.slice(0, 1).toUpperCase() + trt.slice(1)
	...
	const handleInput = (msg) => {
		...
		case "!dmg":
			...
			if (playerIsGM(msg.playerid) && chars) {
				...
				_.each(chars, (char) => {
					if (adjustDamage(char, trait, dtype, dmg)) { D.Alert(`Dealt ${D.JS(dmg)} ${D.JS(dtype)} ${D.JS(trait)} damage to ${D.GetName(char)}`, "CHARS") } else { D.ThrowError(`FAILED to damage ${D.GetName(char)}`, "CHARS") }
				})
			} else {
				D.ThrowError("Select character tokens first!", "CHARS")
			}
		...
		case "!getStat":
			...
			break
		case "!getIncap":
			if (playerIsGM(msg.playerid) && msg.selected && msg.selected[0]) { D.Alert(`Incapacitation String for '${D.GetName(D.GetChar(msg))}': <br/><br/>${D.GetStat(msg, "incap").get("current")}<br/><br/>e.g. !setIncap Compulsion (Arrogance):a:-2<br/><br/>OR<br/><br/>!setIncap Compulsion (Arrogance):Strength,Dexterity,Animal Ken,Dominate:-2`, "CHARS") } else { D.Alert("Select a character first!", "CHARS") }
			break
		case "!setIncap":
			if (playerIsGM(msg.playerid) && msg.selected && msg.selected[0]) {
		...
		case "!setProj":
			...
			break
		case "!copyRepRow":
			if (playerIsGM(msg.playerid) && msg.selected && msg.selected[0] && args.length === 3) { D.CopyToSec(msg, ...args) }
			break
		case "!sortRepRow":
			if (playerIsGM(msg.playerid) && msg.selected && msg.selected[0] && args.length === 2) { D.SortRepSec(msg, args.shift(), SORTFUNCS[args.shift()]) }
			break
		case "!splitRepSecs":
			if (playerIsGM(msg.playerid) && msg.selected && msg.selected[0] && args.length >= 3) { D.SplitRepSec(msg, args.shift(), args.shift(), SORTFUNCS[args.shift()], args[0] || null) }
			break
		case "!setattr":
			if (playerIsGM(msg.playerid) && msg.selected && msg.selected[0]) { setAttrs(D.GetChar(msg).id, { sandboxquadrant: args.shift() }) }
			break
		case "!MVC":
			params = {
				name: who,
