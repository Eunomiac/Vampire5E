const Chars = function () {
	...
	const MVCVALS = [
	 ...
	 regChars = function (chars) {
			_.each(chars, function register (char) {
	 ...
	 getChars = function () {
			const charArray = []
			_.each(state[D.GAMENAME].Chars, function findChar (v, k) {
				if (k === v.id)
	 ...
	 getAttr = function (charObj, attr) {
			const parsedAttr = attr.toLowerCase().replace(/\s+/gu, "")
				.replace(/ /gu, "")

			return filterObjs(function isObjOK (obj) {
	 ...
	 adjustDmg = function (charObj, trait, dtype, amount) {
			const attrList = {},
		 casedTrait = trait.slice(0, 1).toUpperCase() + trait.slice(1)
			attrList[casedTrait.toLowerCase() + ( ["superficial", "superficial+", "spent"].includes(dtype) ? " sDmg" : " aDmg")] = parseInt(amount) > 0 && dtype === "superficial" ? parseInt(Math.ceil(amount / 2)) : parseInt(amount)
	 ...
	 handleInput = function (msg) {
		 ...
		 args = msg.content.split(/\s+/u)
			...
			case "!rChar": {
				...
				if (msg.selected && msg.selected[0] )
					regChars(D.GetChars(msg))
				else
					D.Alert("Select character tokens first!")
				break
			} case "!xp": {
				// !xp Cost Session Message, with some character tokens selected.
				if (!playerIsGM(msg.playerid))
					return
			 ...
			 session = args.shift()
				_.each(chars, function giveXP (char) {
				...
				const [trait, dtype] = [args.shift(), args.shift()],
			 dmgAmount = parseInt(args.shift())
				_.each(chars, function dealDmg (char) {
					if (adjustDmg(char, trait, dtype, dmgAmount))
				...
				break
			} case "!setCompulsion": case "!setComp": {
				if (!playerIsGM(msg.playerid))
					return

				/* Compulsiondisplay toggle, compulsion
           ARROGANCE --- Suffer a -2 penalty to all Social and Mental rolls until the end of the scene OR
           until you give an order that is followed (without using supernatural compulsion). */
				if (msg.selected && msg.selected[0] ) {
				...
				break
			} case "!setDys": case "!setDyscrasias": {
				if (!playerIsGM(msg.playerid))
					return

				/* Dyscrasiasdisplay toggle, dyscrasias
           FRAGILE CONFIDENCE --- Add three dice to all rolls to intimidate or oppress others (including uses of
            Dominate), until someone successfully resists you.  After that point, subtract three dice from all
            Social rolls. */
				if (msg.selected && msg.selected[0] ) {
					const params = args.join(" ").split("|")
					if (params.length !== 2) {
				...
				break
			} case "!clearComp": case "!clearCompulsion": {
				if (!playerIsGM(msg.playerid))
					return
				if (msg.selected && msg.selected[0] ) {
				...
				break
			} case "!clearDys": case "!clearDyscrasias": {
				if (!playerIsGM(msg.playerid))
					return
				if (msg.selected && msg.selected[0] ) {
				...
				if (!playerIsGM(msg.playerid))
					return
				if (msg.selected && msg.selected[0] ) {
					const attrs = _.filter(findObjs( {_type: "attribute", _characterid: D.GetChar(msg).id} ), aObj => aObj.get("name").toLowerCase()
				...
				if (!playerIsGM(msg.playerid))
					return
				if (msg.selected && msg.selected[0] ) {
					let attrs = _.filter(findObjs( {_type: "attribute", _characterid: D.GetChar(msg).id} ), aObj => aObj.get("name").toLowerCase()
						.includes("repeating_project_"))
					_.each(attrs, aObj => aObj.remove())
					attrs = _.filter(findObjs( {_type: "attribute", _characterid: D.GetChar(msg).id} ), aObj => aObj.get("name").toLowerCase()
						.includes("repeating_project_"))
			...
			// D.Log("ID: " + D.JSL(msg));
				...
				_.each(findObjs( {
					...
					type: "text",
					layer: "objects"
				} ), function setAccess (obj) { obj.set("controlledby", "all") } )
				break
			} case "!debug": {
				if (!playerIsGM(msg.playerid))
					return
		...
		// #region Public Functions: initHandlers, tapSpite
