var Roller = Roller || (function () {
	...
	const scaleFrame = function (row, width) {
		...
		for (var i = 0; i < blanks.length; i++) {
			D.DB("Blanking Img #" + imgs.length + i, "SCALEFRAME()", 4);
	...
	const setDie = function (dieNum, dieCat, dieVal, params) {
		...
		if (dieVal == "blank" || dieVal.includes("H") || params.type && params.type != "trait")
	...
	const setText = function (objName, params) {
		...
		if (params.shift && params.shift.anchor) {
			...
			let anchorHeight = parseInt(anchorObj.get("height"));
			let anchorTop = parseInt(anchorObj.get("top"));
	...
	const formatDiceLine = function (rollData, rollResults, splitAt) {
		...
		if (rollData.isReroll) {
			let newResults = rollData.newResults;
			let changeLine = "";
			_.each(rollResults);
