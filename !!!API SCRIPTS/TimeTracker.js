var TimeTracker = TimeTracker || (function () {
    ...
    var setCurrentDate = function (date, tracker, horizon) {
        tracker.set("text", DAYSOFWEEK[date.getUTCDay()] + ", " + MONTHS[date.getMonth()] + " " + D.Ordinalize(date.getUTCDate()) + ", " + (date.getUTCHours() % 12).toString().replace(/^0:/g, "") + ":" + (date.getUTCMinutes() < 10 ? "0" : "") + date.getUTCMinutes().toString() + " " + (Math.floor(date.getUTCHours() / 12) == 0 ? "AM" : "PM"));
        ...
        if (date.getUTCFullYear() != lastDate.getUTCFullYear() || date.getMonth() != lastDate.getMonth() || date.getUTCDate() != lastDate.getUTCDate())
        ...
        switch (args.shift().toLowerCase()) {
            case '!time':
                if (!state[D.GAMENAME].TimeTracker.timeText)
                    return D.Alert("Register a text object first, with '!reg time'", "TIMETRACKER");
                if (!state[D.GAMENAME].TimeTracker.horizonImage)
                    return D.Alert("Register an image object first, with '!reg horizon'", "TIMETRACKER");
            ...
            case '!reg':
                if (!msg.selected || !msg.selected[0])
                    return D.ThrowError("Select an object, then '!reg <time/horizon>'.");
                switch (args.shift().toLowerCase()) {
                    case 'time':
