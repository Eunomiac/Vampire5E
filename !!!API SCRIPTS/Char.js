...
const Char = (() => {
    ...
    const initialize = () => {
        ...
        //STATEREF.registry["1"].playerID = "-LLIBpH_GL5I-9lAOiw9"

        // Return Player Control:
        STATEREF.registry["4"].playerID = "-LMGDbZCKw4bZk8ztfNf"
        STATEREF.registry["3"].playerID = "-LN7lNnjuWmFuvVPW76H"
        STATEREF.registry["2"].playerID = "-LN6n-fR8cSNR2E_N_3q"
        STATEREF.registry["1"].playerID = "-LMGDQqIvyL87oIfrVDX"
    }
    // #endregion	
    ...
    const SORTFUNCS = {
        ...
        registerChar = (msg, num) => {
            ...
            D.Alert(`Character #${D.JSC(charNum)} Registered:<br><br>${D.JS(REGISTRY[charNum])}`, "CHARS:RegisterChar")
            return true
        },
        ...
        awardXP = (charRef, award, reason) => {
            ...
            return D.ThrowError(`Unable to make row for '${D.JSC(char)}'`, "AWARDXP")
        },
        ...
        handleInput = (msg, who, call, args) => { 	// eslint-disable-line no-unused-vars
            let [chars, params, attrList] = [[], [], []],
                charData = {},
                [amount, dmg] = [0, 0],
                [trait, dtype, attrString, playerObj, macroObjs, alertString, msgString] = new Array(7).fill(""),
                char = null
            switch (call) {
                case "reg":
                    if (args.shift() === "token") 
                ...
                case "xp": // !char xp Cost Session Message, with some character tokens selected.
                    ...
                    break
                case "dmg": case "damage": case "heal": case "spend":
                    ...
                    switch (args.shift().toLowerCase()) {
                        case "stat":
                            ...
                            for (const char of D.GetChars("registered"))
                                params.push(`${char.get("name")}: ${D.GetStat(char, trait) ? D.GetStat(char, trait).get("current") : "-"}`)
                ...
                case "set":
                    switch (args.shift().toLowerCase()) {
                        case "attr": case "attrs": case "stat": case "stats":
                    ...
                    break
                case "rep": case "repeating": case "repeat":
                    switch (args.shift().toLowerCase()) {
                        case "copy":
                        case "copyrow":
                    ...
                    break
                case "defaults": populateDefaults(args.shift()); break
                case "setnpcs": setNPCStats(args.shift()); break
                case "changeattr": changeAttrName(args.shift(), args.shift()); break
                default: break
            }
        }
    // #endregion
