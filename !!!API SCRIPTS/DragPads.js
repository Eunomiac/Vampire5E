const DragPads = (() => {
	...
	const IMAGES = {
		...
		makePad = function (graphicObj, funcName, params = "height:0, width:0, x:0, y:0") {
			...
			_.each(params.split(/,\s*?(?=\S)/gu),
				v => { [, options[v.split(/\s*?(?=\S):\s*?(?=\S)/gu)[0]]] = v.split(/\s*?(?=\S):\s*?(?=\S)/gu) } )
		...
		partnerPad = padID => {
			...
			if (graphicObj) {
				...
				pPad.set( {layer: "gmlayer", controlledby: ""} )
		...
		togglePad = function (padRef, isActive) {
			...
			for (const pID of padIDs) {
				...
				if (D.IsObj(pad, "graphic") && D.IsObj(partner, "graphic")) {
					pad.set( {layer: isActive && state[D.GAMENAME].DragPads.byPad[pad.id].active === "on" ? "objects" : "map"} )
					partner.set( {layer: isActive && state[D.GAMENAME].DragPads.byPad[partner.id].active === "on" ? "objects" : "map"} )
				}
			}
		...
		setPad = function (graphicId, params) {
			...
			const [pad] = findObjs( {_id: state[D.GAMENAME].DragPads.byGraphic[graphicId].id} )
			if (!pad)
		...
		handleMove = function (obj) {
			if (obj.get("layer") === "gmlayer" || !state[D.GAMENAME].DragPads.byPad[obj.id] )
				return false
			// toggle object
			obj.set( {layer: "gmlayer", controlledby: ""} )
			...
			partnerObj.set( {layer: "objects", controlledby: "all"} )
		...
		handleInput = function (msg) {
			...
			case "!wpCLEAR":
				...
				state[D.GAMENAME].DragPads = {byPad: {}, byGraphic: {} }
				break
			default:
				break
			}
		...
		checkInstall = () => {
			...
			state[D.GAMENAME].DragPads = state[D.GAMENAME].DragPads || {byPad: {}, byGraphic: {} }
		}
		// #endregion
