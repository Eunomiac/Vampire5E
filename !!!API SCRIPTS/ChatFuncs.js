...
var ChatFuncs = ChatFuncs || (function () {
    'use strict';
    ...
    const getCharAttrs = function (obj) {
        if (!obj)
            return false;
        const allAttrObjects = findObjs({ _type: "attribute", _characterid: obj.get("represents") });
        let allAttrs = [];
        _.each(allAttrObjects, function (attrObj) {
            allAttrs.push({ name: D.JS(attrObj.get("name")), current: D.JS(attrObj.get("current")).replace(/\[/g, "\\[").replace(/{/g, "\\{") });
        });
        const sortedAttrs = _.sortBy(allAttrs, "name");
        let attrsLines = [];
        _.each(sortedAttrs, function (attrInfo) {
            attrsLines.push("<b>" + D.JS(attrInfV.name).replace(/''/g, "") + ":</b> " + D.JS(attrInfV.current).replace(/\\\\/g, "\\").replace(/''/g, ""));
        });
    ...
    const prepText = function (objIDs, string) {
        for (var i = 0; i < objIDs.length; i++) {
            ...
            var obj = findObjs({ _id: objIDs[i]._id })[0];
    ...
    const handleInput = function (msg) {
        ...
        switch (args.shift()) {
            ...
            case '!set':
                ...
                switch (args.shift()) {
                    case 'size':
                        ...
                        let initX = parseInt(obj.get("width"));
                        let initY = parseInt(obj.get("height"));
                        let attrList = {
                        ...
                        var [X, Y] = [parseInt(1), parseInt(2)];
                        D.Alert("Deconstructed: " + D.JS(X));
                        break;
                        obj.set();
                        break;
                }
            ...
            case '!find':
                switch (args.shift()) {
                    ...
                    case 'textWidth':
                        if (!msg.selected || !msg.selected[0])
                            break;
                        let width = D.GetTextWidth(findObjs({ _id: msg.selected[0]._id })[0], args.join(" "));
            ...
            case '!checkText':
                if (!msg.selected || !msg.selected[0])
                    break;
                let thisObj = findObjs({ _id: msg.selected[0]._id })[0];
