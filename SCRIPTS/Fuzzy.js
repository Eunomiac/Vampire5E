...
const Fuzzy = (() => {
        ...
        switch (call) {
            ...
            case "get": {
                switch (D.LCase((call = args.shift()))) {
                    ...
                    default: {
                        D.Alert(
                            `Fuzzy Minimum Match Score is <b>${STATE.REF.minMatchScore}</b><br><br>Default = 0.33; <b>!fuzzy set minmatch &lt;#&gt;</b> to change.`,
                            "!fuzzy set minmatch"
                        );
                        break;
                    }
    ...
    const Fix = function(dict) {
        ...
        const fuzzyset = {
            ...
            _iterateGrams = function(value, gramSize) {
                ...
                let simplified = `-${value.toLowerCase().replace(_nonWordRe, "")}-`,
                    ...
                    results = [];
                if (lenDiff > 0) for (let i = 0; i < lenDiff; ++i) simplified += "-";

                for (let i = 0; i < simplified.length - gramSize + 1; ++i) results.push(simplified.slice(i, i + gramSize));
            ...
            _gramCounter = function(value, gramSize) {
                ...
                for (i; i < grams.length; ++i)
                    if (grams[i] in result) result[grams[i]] += 1;
        ...
        fuzzyset.get = function(value, defaultValue, minMatchScore) {
            ...
            if (minMatchScore === undefined) minMatchScore = 0.33;

            const result = this._get(value, minMatchScore);
            if (!result && typeof defaultValue !== "undefined") return defaultValue;
        ...
        fuzzyset.__get = function(value, gramSize, minMatchScore) {
            ...
            if (this.useLevenshtein) {
                ...
                for (let i = 0; i < endIndex; ++i) newResults.push([_distance(results[i][1], normalizedValue), results[i][1]]);
            ...
            results.forEach(
                function(scoreWordPair) {
                    if (scoreWordPair[0] >= minMatchScore) newResults.push([scoreWordPair[0], this.exactSet[scoreWordPair[1]]]);
                }.bind(this)
            );
        ...
        fuzzyset.add = function(value) {
            const normalizedValue = this._normalizeStr(value);
            if (normalizedValue in this.exactSet) return false;

            let i = this.gramSizeLower;
            for (i; i < this.gramSizeUpper + 1; ++i) this._add(value, i);
        };
        ...
        fuzzyset._add = function(value, gramSize) {
            ...
            for (gram in gramCounts) {
                gramCount = gramCounts[gram];
                sumOfSquareGramCounts += Math.pow(gramCount, 2);
                if (gram in this.matchDict) this.matchDict[gram].push([index, gramCount]);
        ...
        fuzzyset.length = function() {
            let count = 0,
                prop;
            for (prop in this.exactSet) if (this.exactSet.hasOwnProperty(prop)) count += 1;
        ...
        fuzzyset.isEmpty = function() {
            for (const prop in this.exactSet) if (this.exactSet.hasOwnProperty(prop)) return false;
        ...
        fuzzyset.values = function() {
            let values = [],
                prop;
            for (prop in this.exactSet) if (this.exactSet.hasOwnProperty(prop)) values.push(this.exactSet[prop]);
        ...
        for (i; i < fuzzyset.gramSizeUpper + 1; ++i) fuzzyset.items[i] = [];

        // add all the items to the set
        for (i = 0; i < dict.length; ++i) fuzzyset.add(dict[i]);
