...
const Media = (() => {
    ...
    const initialize = () => {
        // STATE.REF.imgregistry["MapButton_Domain_1"].cycleSrcs = [ "base", "anarch", "camarilla", "autarkis", "secondinquisition" ];

        // ["CN-LED-1", "CN-LED-2", "CN-LED-3", "CN-LED-4"].forEach((x) => delete STATE.REF.animregistry[x]);

        // const delImgObj = getImgObj("WeatherCNTowerClouds_1");
        // delete STATE.REF.imgregistry.WeatherCNTowerClouds_1;
        // delImgObj.remove();
        /* STATE.REF.animregistry["CN-LED-1"].top = 273;
            STATE.REF.animregistry["CN-LED-1"].left = 1226;

            STATE.REF.animregistry["CN-LED-1"].leftEdge = 1200.5;
            STATE.REF.animregistry["CN-LED-1"].rightEdge = 1251.5;
            STATE.REF.animregistry["CN-LED-1"].topEdge = 0;
        ...
        for (const [, textData] of Object.entries(STATE.REF.textregistry)) {
            ...
            for (const [, modeData] of Object.entries(textData.modes)) {
                if (modeData.lastState === "LAST") { modeData.lastState = realText }
            }
        }
        ...
        const NPCTOKENSTARTPOS = {
            TopLeft: {left: 53, top: 522},
            BotLeft: {left: 277, top: 754},
            TopRight: {left: 1535, top: 522},
            MidRight: {left: 1434, top: 705},
            BotRight: {left: 1313, top: 754}
        };

        Object.assign(STATE.REF.areas.npcTokenTopLeft, NPCTOKENSTARTPOS.TopLeft);
        Object.assign(STATE.REF.areas.npcTokenBotLeft, NPCTOKENSTARTPOS.BotLeft);
        Object.assign(STATE.REF.areas.npcTokenTopRight, NPCTOKENSTARTPOS.TopRight);
        Object.assign(STATE.REF.areas.npcTokenBotRight, NPCTOKENSTARTPOS.BotRight);
        STATE.REF.areas.npcTokenMidRight = {top: 705, left: 1434, height: 100, width: 90};
        /*
                ...
                switch (D.LCase((call = args.shift()))) {
                    ...
                    case "set": {
                        switch (D.LCase((call = args.shift()))) {
                            ...
                            case "zindex": {
                                const [mediaObj] = mediaObjs;
                                const zIndex = D.Int(args.shift());
                                const mediaData = getMediaData(mediaObj);
                                DB({mediaObj, zIndex, mediaData}, "setzindex");
                                if (VAL({int: zIndex, list: mediaData})) {
                                    DB({setZIndex: true}, "setzindex");
                                    setMediaData(mediaObj.id, {zIndex});
                                    setZIndices();
                                }
                                break;
                            }
                    ...
                    case "fix": {
                        switch (D.LCase((call = args.shift()))) {
                            ...
                            case "positions": {
                                fixImgPositions(!args.includes("confirm"));
                                break;
                            }
                            case "zlevels":
                            case "zindex":
                            ...
                            case "all": {
                                fixAll(args.join(" ").includes("kill"), args.join(" ").includes("test"));
                                break;
                            }
                            default: {
                                D.Alert(
            ...
            case "!areas": {
                ...
                switch (D.LCase((call = args.shift()))) {
                    ...
                    case "register": {
                        ...
                        if (args.length && VAL({imgObj}, "!area register")) { regArea(imgObj, args.shift()) } else { D.Alert("Syntax: !area reg &lt;areaName&gt;", "!area reg") }
                        break;
                    }
                    case "get": {
                        switch (D.LCase((call = args.shift()))) {
                ...
                switch (D.LCase((call = args.shift()))) {
                    ...
                    case "get": {
                        switch (D.LCase((call = args.shift()))) {
                            ...
                            case "charsin": {
                                ...
                                for (const imgObj of imgObjs) { containedCharObjs.push(...Media.GetContainedChars(getImgKey(imgObj), {padding: 50})) }
                            ...
                            switch (D.LCase((call = args.shift()))) {
                                ...
                                case "gentoken": {
                                    ...
                                    for (const img of imgObjs) {
                                        if (VAL({img})) { regGenericToken(img, tokenCat) }
                                    }
                                    break;
                                }
                                case "random":
                    ...
                    case "set": {
                        switch (D.LCase((call = args.shift()))) {
                            case "tokenscale": {
                                ...
                                imgObjs.forEach((img) => {
                                    setImgData(img, {tokenScale: D.Float(scaleFactor || 1, 2)});
                                });
                                break;
                            }
                            case "mode": {
                                ...
                                if (!Session.Modes.includes(mode)) {
                                    ...
                                    for (const key of Object.keys(params)) {
                                        if (["true", "false", "null"].includes(D.LCase(params[key]))) { params[key] = {true: true, false: false, null: null}[D.LCase(params[key])] }
                            ...
                            case "src": {
                                ...
                                if (isRegImg(hostName)) { setImg(hostName, srcName) } else { D.Alert(`Img name ${D.JS(hostName)} is not registered.`, "MEDIA: !img set src") }
                                break;
                            }
                            case "tokensrc": {
                                const charObjs = Listener.GetObjects(objects, "characters");
                                for (const charObj of charObjs) {
                                    if (VAL({charObj})) { combineTokenSrc(charObj, args.join(" ")) }
                                }
                                break;
                            }
                            case "activesource":
                            case "activesrc": {
                                const [imgObj] = imgObjs;
                                const [hostName, srcName] = [getImgKey(imgObj), args.shift()];
                                if (isRegImg(hostName)) { setImgData(hostName, {activeSrc: srcName}) } else { D.Alert(`Img name ${D.JS(hostName)} is not registered.`, "MEDIA: !img set activesrc") }
                                break;
                            }
                            case "area": {
                                const [imgObj] = imgObjs;
                        ...
                        switch (D.LCase((call = args.shift()))) {
                            ...
                            case "source": {
                                ...
                                if (VAL({string: srcName}, "!img add cyclesrc") && isRegImg(hostName)) {
                                    ...
                                    if (srcName) { addImgSrc(msg, hostName, srcName) } else { D.Alert(`Invalid image name '${D.JS(srcName)}'`, "MEDIA: !img add src") }
                                } else {
                            ...
                            case "tokensource": {
                                ...
                                if (VAL({charObj, imgObj: tokenSrcObj, string: tokenSrcName})) { addTokenSrc(tokenSrcObj, charObj, tokenSrcName) } else {
                                    D.Alert(
                        ...
                        switch (D.LCase((call = args.shift()))) {
                            ...
                            default: {
                                ...
                                for (const arg of args) {
                                    if (arg in REGISTRY.IMG) { delete REGISTRY.IMG[arg] } else if (`${arg}_1` in REGISTRY.IMG) { delete REGISTRY.IMG[`${arg}_1`] }
                                }
                                break;
                            }
                        }
                    ...
                    case "toggle": {
                        switch (D.LCase((call = args.shift()))) {
                            ...
                            case "log": {
                                imgRecord = !imgRecord;
                                if (imgRecord) { D.Alert("Logging image data as they are added to the sandbox.", "MEDIA, !img toggle log") } else { D.Alert("Img logging disabled.", "MEDIA, !img toggle log") }
                                break;
                            }
                            case "resize": {
                                const params = D.ParseParams(args);
                            ...
                            case "autotoken": {
                                if (args[0] === "skip") {
                                    ...
                                    if (imgSrcAutoToken) {
                                        ...
                                        if (VAL({number: args[0]})) { STATE.REF.autoRegTokenNames = STATE.REF.autoRegTokenNames.slice(D.Int(args[0])) }
                                        D.Alert(
                    ...
                    case "unregister": {
                        switch (D.LCase((call = args.shift()))) {
                            ...
                            default: {
                                ...
                                for (const arg of args) {
                                    if (arg in REGISTRY.TEXT) { delete REGISTRY.TEXT[arg] }
                                }
                                break;
                            }
                        }
            ...
            case "!anim": {
                ...
                switch (D.LCase((call = args.shift()))) {
                    ...
                    case "toggle": {
                        switch (D.LCase((call = args.shift()))) {
                            case "on": {
                                for (const imgObj of imgObjs) { toggleAnimation(imgObj, true) }
                                break;
                            }
                            case "off": {
                                for (const imgObj of imgObjs) { toggleAnimation(imgObj, false) }
                                break;
                            }
                            default: {
                                D.Alert(
    ...
    const onGraphicAdd = (imgObj) => {
        ...
        if (imgResize) { trySet(imgObj, STATE.REF.imgResizeDims) }
        ...
        if (imgSrcAutoGeneric && !isRegImg(imgObj) && !isCharToken(imgObj)) {
            if (regGenericToken(imgObj, imgSrcAutoGeneric)) { imgObj.remove() }
        }
    ...
    const onGraphicChange = (imgObj, prevData) => {
        ...
        if (VAL({tokenObj: imgObj})) { scaleImgObjs(imgObj) }
        if (STATE.REF.areImgsBound) {
            let imgKey = getImgKey(imgObj);
            // DB({imgKey}, "onGraphicChange");
            if (VAL({tokenObj: imgObj})) { imgKey = imgObj.get("name") }
            // DB({imgKey}, "onGraphicChange");
    ...
    const IMGDIMS = {
        token: {height: 100, width: 90},
        genericToken: {height: 73, width: 90}
    };
    ...
    const isRegistered = (mediaRef) => {
        ...
        return (isRegText(mediaRef) && "text")
                || (isRegImg(mediaRef) && "img")
                || (isRegAnim(mediaRef) && "anim");
    };
    ...
    const getMediaObj = (mediaRef) => {
        ...
        const mediaKey = getMediaKey(mediaRef);
        if (isRegistered) {
    ...
    const getMediaPos = (mediaRef) => {
        if (isRegistered(mediaRef)) {
            const allData = getMediaData(mediaRef);
            const posData = _.mapObject(_.pick(allData, ["left", "top", "height", "width", "leftEdge", "rightEdge", "topEdge", "bottomEdge"]), (v) => Math.floor(v));
            DB({mediaRef, posData}, "getPosData");
            if (("shiftTop" in allData) || ("pushtop" in allData)) { posData.top += D.Float(allData.shiftTop, 2) + D.Float(allData.pushtop, 2) }
            if (("shiftLeft" in allData) || ("pushleft" in allData)) { posData.left += D.Float(allData.shiftLeft, 2) + D.Float(allData.pushleft, 2) }
            // IF object is inactive, set height and width to 0 for return data:
            if (!isObjActive(mediaRef)) {
                STATE.REF.layoutReportLines.push(`...getMediaPos(${D.JSL(mediaRef)}) -> isObjActive() = ${D.JSL(isObjActive(mediaRef))}: INACTIVE, zeroing posData`);
                posData.height = 0;
                posData.width = 0;
                posData.rightEdge = posData.leftEdge;
                posData.bottomEdge = posData.topEdge;
            }
            return posData;
        } else if (VAL({obj: mediaRef})) {
            const posData = {
                left: D.Float(mediaRef.get("left"), 2),
                top: D.Float(mediaRef.get("top"), 2),
                height: D.Float(mediaRef.get("height"), 2),
                width: D.Float(mediaRef.get("width"), 2)
            };
            ...
            if (mediaRef.get("layer") === "walls") {
                STATE.REF.layoutReportLines.push(`...getMediaPos(${D.JSL(mediaRef)}) -> ON WALLS, zeroing posData`);
                posData.height = 0;
                posData.width = 0;
                posData.rightEdge = posData.leftEdge;
                posData.bottomEdge = posData.topEdge;
            ...
            return THROW(`Error finding position data for ${D.JSL(mediaRef)}`, "getPosData");
        }
    };
    const hasForcedState = (mediaRef) => {
        ...
        if (VAL({list: mediaData}, "hasForcedState")) { return TRACEOFF(traceID, VAL({string: mediaData.isForcedState}) && mediaData.isForcedState !== "LAST") }
        ...
        if (isRegistered(mediaRef)) {
            ...
            if (VAL({list: mediaData}, "getModeStatus")) {
                ...
                if (VAL({list: mediaModes}, "getModeStatus")) {
                    if (mediaModes.isForcedOn === "LAST") { modeStatus.isActive = mediaModes.lastActive } else if (mediaModes.isForcedOn === "NEVER") { modeStatus.isActive = false } else if (mediaModes.isForcedOn === true || mediaModes.isForcedOn === false) { modeStatus.isActive = mediaModes.isForcedOn } else { modeStatus.isActive = mediaData.isActive }
                    if (mediaModes.isForcedState === true || mediaModes.isForcedState === "LAST") { modeStatus.state = mediaModes.lastState } else if (mediaModes.isForcedState === null) { modeStatus.state = undefined } else { modeStatus.state = mediaModes.isForcedState }
    ...
    const trySet = (mediaObj, params) => {
        if (VAL({obj: mediaObj, list: params}, "trySet")) {
            ...
            ["left", "top", "height", "width"].forEach((x) => {
                if (x in params && !VAL({number: params[x]})) { errorLog.push(`SET ERROR: [${mediaObj.get("name")}] {${x}: ${D.JSL(params[x])}}`) }
            });
            const setParams = D.FilterForObjAttrs(mediaObj, params);
            mediaObj.set(setParams);
            if (errorLog.length) { DB({errorLog}, "trySet") }
            return true;
        }
        return false;
    };
    const fixAll = (isKilling = false, isTestRun = false) => {
    ...
    const setMediaTemp = (mediaRef, params = {}) => {
        ...
        const mediaObj = getMediaObj(mediaRef);
        if (VAL({object: mediaObj})) { trySet(mediaObj, params) }
        TRACEOFF(traceID);
    };
    const setMediaData = (mediaRef, params = {}) => {
        DB({setMediaData: true, isRegistered: isRegistered(mediaRef)}, "setzindex");
        switch (isRegistered(mediaRef)) {
            case "img": return setImgData(mediaRef, params, true);
            case "text": return setTextData(mediaRef, params);
            case "anim": return setAnimData(mediaRef, params);
    ...
    const toggle = (mediaRef, isActive, isForcing = false) => {
        ...
        if (isRegAnim(mediaRef)) { return TRACEOFF(traceID, toggleAnimation(mediaRef, isActive)) }
    ...
    const setActiveLayers = () => {
        ...
        for (const objData of mediaObjData) { trySet(objData[0], {layer: (objData[1].isActive && objData[1].activeLayer) || "walls"}) }
        TRACEOFF(traceID);
    };
    const setZIndices = () => {
    ...
    const resetModeData = (isResettingAll = false, isQueueing = false, isVerbose = false, isChangingData = false) => {
        ...
        if (errorLines.length) {
            if (isQueueing) { STATE.REF.fixAllCommands.push(...["<h3><u>Resetting Mode Data</u></h3>", ...errorLines]) } else {
                D.Alert(
                    _.compact([
    ...
    const isGenericToken = (imgObj) => {
        const tokenCat = D.LCase(D.GetStatVal(imgObj, "tokencat"));
        return tokenCat in REGISTRY.TOKEN.GENERIC;
    };
    ...
    const getImgObjs = (imgRefs, funcName = false) => {
        ...
        if (VAL({array: imgRefs})) {
            for (const imgRef of imgRefs) { imgObjs.push(getImgObj(imgRef, funcName)) }
        }
    ...
    const getImgData = (imgRef, funcName = false) => {
        ...
        const imgData = (() => {
            ...
            try {
                ...
                if (VAL({string: imgKey}) || (VAL({imgObj: imgKey}) && REGISTRY.GRAPHIC[imgKey.get("name")])) { return TRACEOFF(innerTraceID, REGISTRY.GRAPHIC[imgKey] || REGISTRY.GRAPHIC[imgKey.get("name")]) }
    ...
    const getContainedImgObjs = (containerRef, options = {}, filter = {}) => {
        ...
        const containedImgObjs = findObjs({
            ...
            for (const [prop, value] of Object.entries(filter)) {
                if (x.get(prop) !== value) { return false }
            }
    ...
    const addImgSrc = (imgSrcRef, imgName, srcName, isSilent = false) => {
        ...
        try {
            ...
            if (imgSrc !== "" && isRegImg(imgName)) {
                if (srcName.startsWith("ref:")) { REGISTRY.IMG[getImgKey(imgName)].srcs = srcName.replace(/ref:/gu, "") } else { REGISTRY.IMG[getImgKey(imgName)].srcs[srcName] = imgSrc }
    ...
    const setGenericToken = (imgObj) => {
        ...
        const tokenCat = D.LCase(D.GetStatVal(imgObj, "tokencat"));
        const tokenName = D.GetName(imgObj.get("represents"));
    ...
    const toggleTokens = (tokenRef, isActive) => {
        ...
        if (isActive !== null) {
            for (const tokenObj of getTokenObjs(tokenRef)) {
                if (tokenObj.get("layer") === "gmlayer") { continue }
                for (const tokenAura of getActiveTokenAuras(tokenRef)) { toggle(tokenAura, isActive) }
                if (isActive === true && tokenObj.get("layer") !== "objects") { trySet(tokenObj, {layer: "objects"}) } else if (isActive === false && tokenObj.get("layer") !== "walls") { trySet(tokenObj, {layer: "walls"}) }
            }
        }
        TRACEOFF(traceID);
    };
    const combineTokenSrc = (charRef, srcName = "base") => {
        ...
        if (VAL({tokenObj, string: srcName}) && isRegToken(tokenObj)) {
            ...
            let newTokenSrcs = [];
            if (D.LCase(srcName) === "base") { newTokenSrcs = ["base"] } else if (splitTokenSrcs.includes(D.Capitalize(srcName))) { newTokenSrcs = _.without(splitTokenSrcs, D.Capitalize(srcName)) } else { newTokenSrcs = [..._.without(splitTokenSrcs, "Base"), D.Capitalize(srcName)] }
            newTokenSrcs.sort();
            DB(
                {
                    tokenName,
    ...
    const getActiveTokenModes = (charRef) => {
        ...
        if (tokenSrc) { return D.Capitalize(tokenSrc).match(/[A-Z][a-z]*/gu) }
        return [];
    };
    const getActiveTokenAuras = (charRef) => {
        ...
        if (tokenSrc) {
            ...
            for (const srcName of splitTokenSrcs) {
                if (D.Capitalize(srcName) in STATE.REF.tokenAuras) { tokenAuras.push(STATE.REF.tokenAuras[srcName]) }
            }
    ...
    const regImg = (imgRef, imgName, srcName, activeLayer, options = {}, funcName = false, isSilent = false) => {
        ...
        if (VAL({graphicObj: imgObj}, "regImg")) {
            ...
            if (!params.left || !params.top || !params.height || !params.width) { return TRACEOFF(traceID, THROW("Must supply position & dimension to register image.", "RegImg")) }
            ...
            if (VAL({string: funcName}) && !isSilent) { D.Alert(`Host obj for '${D.JS(name)}' registered: ${D.JS(REGISTRY.IMG[name])}`, "MEDIA: regImg") }
            ...
            if (VAL({string: srcRef})) {
                ...
                if (VAL({string: srcURL}, "setImg")) {
                    if (VAL({imgObj}, ["setImg", `Key: ${D.JS(imgData.name)}`])) {
                        ...
                        if (imgData.name in REGISTRY.IMG) {
    ...
    const cycleImg = (imgRef, isLooping = true, isReversing = false) => {
        ...
        if (imgData && isCyclingImg(imgData.id)) {
            ...
            if (newIndex < 0) {
                if (isLooping) { newIndex = imgData.cycleSrcs.length - 1 } else { return TRACEOFF(traceID, false) }
            } else if (newIndex >= imgData.cycleSrcs.length) {
                if (isLooping) { newIndex = 0 } else { return TRACEOFF(traceID, false) }
            }
    ...
    const setImgTemp = (imgRef, params) => {
        ...
        if (VAL({imgObj}, "setImgTemp")) {
            ...
            if ("top" in params || "shiftTop" in params) {
                params.top = (VAL({number: params.top}) ? D.Int(params.top) : D.Int(imgData.top))
                        + (VAL({number: params.shiftTop}) ? D.Int(params.shiftTop) : D.Int(imgData.shiftTop));
            }
            if ("left" in params || "shiftLeft" in params) {
                params.left = (VAL({number: params.left}) ? D.Int(params.left) : D.Int(imgData.left))
                        + (VAL({number: params.shiftLeft}) ? D.Int(params.shiftLeft) : D.Int(imgData.shiftLeft));
            }
            trySet(imgObj, params);
            if (imgData.padID) { DragPads.Sync(imgData.id) }
            return TRACEOFF(traceID, imgObj);
        }
        return TRACEOFF(traceID, false);
    };
    const setImgData = (imgRef, params, isSettingObject = true) => {
        ...
        if (VAL({string: imgKey}, "setImgData")) {
            _.each(params, (v, k) => {
                if (imgKey in REGISTRY.IMG) { REGISTRY.IMG[imgKey][k] = v } else if (imgKey in REGISTRY.ANIM) { REGISTRY.ANIM[imgKey][k] = v }
            });
    ...
    const resizeImgs = (imgRefs, axes = []) => {
        ...
        for (const imgObj of imgObjs) {
            ...
            if (axes.length === 1) {
                if (axes.includes("horiz") || axes.includes("width")) { dims.height = imgObj.get("height") * (dims.width / imgObj.get("width")) } else if (axes.includes("vert") || axes.includes("height")) { dims.width = imgObj.get("width") * (dims.height / imgObj.get("height")) }
            }
            trySet(imgObj, dims);
        }
        TRACEOFF(traceID);
    ...
    const toggleImg = (imgRef, isActive, isForcing = false) => {
        ...
        if (isRegAnim(imgRef)) { return TRACEOFF(traceID, toggleAnimation(imgRef, isActive)) }
        ...
        if (VAL({list: [imgData, modeData]}, "toggleImg", true)) {
            ...
            if ((isActive === true || (isActive !== false && !imgData.isActive)) && modeData.isForcedOn !== "NEVER") { activeCheck = true } else if (isActive === false || (isActive !== true && imgData.isActive) || (modeData.isForcedOn === "NEVER" && imgData.isActive)) { activeCheck = false }
            ...
            if (activeCheck === false) {
                ...
                if (REGISTRY.IMG[imgData.name]) { regRef[imgData.name].activeSrc = (imgData.curSrc === "blank" && imgData.activeSrc) || imgData.curSrc }
    ...
    const checkDragPads = () => {
        ...
        if (
            ...
            const extraDragPadIDs = allPadIDs.filter((padid) => !Object.values(mediaDragPadIDs).flat().includes(padid)).map((id) => `${id}: ${getObj("graphic", id).get("name")}`);
            reportLines.push(...missingDragPadIDs.map((x) => `<b>${DragPads.PadsByID[x].name}</b> (${x})`));
            reportLines.push("<h3>Extra Dragpads</h3>");
            reportLines.push(...extraDragPadIDs);
            D.Alert(
    ...
    const clearUnregImgs = (isKilling = false, isQueueing = true) => {
        ...
        const returnLines = [];
        const isKillingNonGMLayer = Boolean(isKilling);
        const isKillingAll = isKilling === "all";
        const allImgObjs = findObjs({
            _type: "graphic"
        });
        ...
        for (const imgObj of unregImgObjs) {
            ...
            const imgLayer = imgObj.get("layer");
            if (imgObj.get("pageid") !== C.PAGES.GAME
                    || (!isKillingAll && imgLayer === "gmlayer")) { continue }
            ...
            if (isKillingAll || (isKillingNonGMLayer && imgLayer !== "gmlayer")) { imgObj.remove() }
        }
        if (returnLines.length) {
            if (isQueueing) { STATE.REF.fixAllCommands.push(...["<h4><u>Clearing Unregistered Image Objects</u></h4>", ...returnLines]) } else { D.Alert(D.JS(["<h4><u>Clearing Orphan Images</u></h4>", ...returnLines].join("")), "clearUnregImgs") }
        }
        TRACEOFF(traceID);
    };
    ...
    const fixImgPositions = (isTesting = true) => {
        const reportLines = [];
        for (const [imgKey, imgData] of Object.entries(REGISTRY.GRAPHIC)) {
            const updateData = {};
            const imgObj = getObj("graphic", imgData.id);
            if (imgObj) {
                ["left", "top", "height", "width"].forEach((param) => {
                    if (D.Int(imgData[param]) !== D.Int(imgObj.get(param))) {
                        reportLines.push(`<b>[${imgKey} - ${param}]</b> Registered: ${imgData[param]} <> Actual: ${imgObj.get(param)}`);
                        Object.assign(updateData, {[param]: imgData[param]});
                    }
                });
                if (!isTesting) { setImgTemp(imgObj, updateData) }
            }
        }
        D.Alert([
            "<h3>Mispositioned Images</h3>",
            ...reportLines
        ].join("<br>"), "Fixing Image Positions");
    };
    const fixImgObjs = (isQueueing = false) => {
            ...
            if (!isRegAnim(imgObj)) {
                ...
                if (srcURL !== realURL) {
                    if (VAL({string: srcURL}) && srcURL.startsWith("http")) {
                        ...
                        } else if (imgData.curSrc !== "TOKENSOURCE") {
                            reportStrings.push(
                                `Image source URL doesn't match registry source (= ${D.JS(
                                    imgData.curSrc
        ...
        if (reportLines.length) {
            if (isQueueing) { STATE.REF.fixAllCommands.push(...["<h3><u>Final Image Object Pass</u></h3>", ...reportLines]) } else { D.Alert(["<h3><u>Fixing Image Objects</u></h3>", ...reportLines].join("<br>"), "fixImgObjs") }
        }
        TRACEOFF(traceID);
    };
    ...
    const setImgArea = (imgRef, areaRef, isResizing = false) => {
        ...
        if (!imgObj) { return TRACEOFF(traceID, D.Alert(`Invalid image reference: ${D.JS(imgRef)}`, "MEDIA: setImgArea")) } else if (!areaData) { return TRACEOFF(traceID, D.Alert(`No area registered as '${D.JS(areaRef)}'`, "MEDIA: setImgArea")) }
        const imgParams = {
            top: areaData.top,
            left: areaData.left
        };
    ...
    const spreadImgs = (leftImgRef, rightImgRef, midImgRefOrRefs, width, minOverlap = 20, maxOverlap = 40) => {
        ...
        if (VAL({list: [leftData, rightData, ...midData], number: [spread]}, "spreadImgs", true)) {
            ...
            dbObj.leftImgLeft = D.Int(leftData.left);
            ...
            if (spread <= leftData.width + rightData.width - maxOverlap) {
                dbObj.widthTooSmall = `Spread <= (leftWidth:${D.Int(leftData.width)} + rightWidth:${D.Int(rightData.width)} - 2 * maxOverlap)`;
                for (const imgData of midData) { toggleImg(imgData.id, false) }
                dbObj.rightImgLeft = D.Int(leftData.rightEdge + 0.5 * rightData.width);
                DB(dbObj, "spreadImgs");
                return TRACEOFF(
                    traceID,
            ...
            dbObj.totalMidWidth = totalMidWidth;
            ...
            const midImgIDs = [];
            dbObj.midImgWidth = midImgWidth;
            dbObj.minCover = minCover;
            dbObj.maxCover = maxCover;
            dbObj.midImgCollection = [];
            ...
            while (coveredSpread < totalMidWidth) {
                if (midData.length) {
                    ...
                    dbObj.midImgCollection.push(`... adding ${getImgKey(_.last(midImgIDs))}. CoveredSpread: ${D.Int(coveredSpread)} (${midData.length} remaining)`);
                } else {
                    dbObj.midImgCollection.push(`Ran out of mid images! ${totalMidWidth - coveredSpread} to cover!`);
            ...
            dbObj.disablingUnused = [];
            while (midData.length) {
                dbObj.disablingUnused.push(`... disabling ${_.last(midData).name}`);
                toggleImg(midData.pop().id, false);
            }
            dbObj.spreadPerImg = spreadPerImg;
            dbObj.totalMidWidth = totalMidWidth;
            dbObj.numMidImgs = midImgIDs.length;
            ...
            dbObj.sideOverlap = `${D.Int(sideOverlap)} = 0.5 * midImgWidth:${D.Int(midImgWidth)} - spreadPerImg:${D.Int(spreadPerImg)}`;
            dbObj.firstMidLeft = firstMidLeft;
            ...
            dbObj.settingMidPositions = [];
            for (const imgID of midImgIDs) {
                setImgTemp(imgID, {left: currentLeft});
                lastRightEdge = D.Int(currentLeft + 0.5 * midImgWidth);
                dbObj.settingMidPositions.push(`... Setting ${getImgKey(imgID)} to {left: ${currentLeft}}. LastRightEdge:${lastRightEdge}`);
            ...
            dbObj.settingRightPosition = `{left: ${leftData.leftEdge + totalMidWidth + leftData.width + rightData.width - 2 * minOverlap - 0.5 * rightData.width}}`;
            DB(dbObj, "spreadImgs");
    ...
    const bindImgs = (imgRefs, isUnbinding = false) => {
        ...
        const imgKeys = imgRefs.map((x) => {
            if (VAL({char: x})) {
                ...
                if (tokenObj) { return tokenObj.get("name") }
                return D.GetName(x);
            }
            return getImgKey(x);
        });
    ...
    const scaleImgObjs = (imgRefs, scaleFactor, baseImgDims) => {
        const imgObjs = _.compact(_.flatten([imgRefs])).map((x) => getImgObj(x));
        for (const imgObj of imgObjs) {
            let thisScaleFactor, thisBaseDims;
            const curDims = {height: D.Int(imgObj.get("height")), width: D.Int(imgObj.get("width"))};
            if (VAL({number: scaleFactor})) {
                thisScaleFactor = scaleFactor;
            } else {
                const [imgContainerData] = _.sortBy(Object.values(REGISTRY.IMG).filter((x) => x.isActive && x.activeLayer === "map" && isInside(x.name, imgObj)), "zIndex").reverse();
                if (imgContainerData) { thisScaleFactor = imgContainerData.tokenScale }
            }
            thisScaleFactor = D.Float(thisScaleFactor || 1);
            if (VAL({list: baseImgDims})) { thisBaseDims = D.Clone(baseImgDims) } else if (isGenericToken(imgObj)) { thisBaseDims = D.Clone(IMGDIMS.genericToken) } else if (isCharToken(imgObj)) { thisBaseDims = D.Clone(IMGDIMS.token) } else { thisBaseDims = D.Clone(curDims) }
            if (VAL({number: thisScaleFactor, list: thisBaseDims})) {
                const newDims = _.mapObject(thisBaseDims, (x) => D.Int(x * D.Float(thisScaleFactor)));
                if (!_.isEqual(curDims, newDims)) { trySet(imgObj, newDims) }
            }
        }
    };
        // #endregion
    ...
    const flashAnimation = (animName) => {
        ...
        if (!animData.validModes.includes(Session.Mode)) {
            ...
            if (animData.soundEffect && animData.validModes.includes(Session.Mode)) { Soundscape.Play(animData.soundEffect, undefined, undefined, false) }
    ...
    const killAnimation = (animObj) => {
        ...
        if (VAL({imgObj: animObj}, "killAnimation")) { trySet(animObj, {layer: "walls"}) }
        TRACEOFF(traceID);
    };
    const killAllAnims = () => {
        const traceID = TRACEON("killAllAnims", []);
        for (const animData of _.values(REGISTRY.ANIM)) { trySet(getObj("graphic", animData.id), {layer: "walls"}) }
        TRACEOFF(traceID);
    };
        // #endregion
    ...
    const addPanelText = (panelKey = "panel", text) => {
        ...
        setTextData(panelKey, {top: PANELTOP});
    ...
    const getTextKey = (textRef, funcName = false) => {
        ...
        try {
            ...
            if (VAL({textObj})) {
                if (REGISTRY.ID[textObj.id]) { return TRACEOFF(traceID, REGISTRY.ID[textObj.id]) }
            }
            return TRACEOFF(
                traceID,
    ...
    const getTextObj = (textRef, funcName = false) => {
        ...
        try {
            let textObj;
            if (VAL({textObj: textRef})) { textObj = textRef } else if (VAL({string: textRef})) {
                if (getTextKey(textRef, funcName)) { textObj = getObj("text", REGISTRY.TEXT[getTextKey(textRef)].id) } else { textObj = getObj("text", textRef) || null }
    ...
    const getTextObjs = (textRefs, funcName = false) => {
        ...
        if (VAL({array: textRefs})) {
            for (const textRef of textRefs) { textObjs.push(getTextObj(textRef, funcName)) }
        }
    ...
    const getTextWidth = (textRef, text, maxWidth = 0, isSilent = true) => {
        ...
        if (VAL({textObj}, "getTextWidth")) {
            ...
            let textLines = [];
            if (maxWidth !== false && maxW) { textLines = _.compact(splitTextLines(textObj, textString, maxW, textData && textData.justification)) } else if (maxWidth !== false && textString && textString.includes("\n")) {
            ...
            for (const char of chars) {
                charString += char;
                if (char !== "\n" && !charRef[char]) { D.MissingTextChars = char } else { width += charRef[char] }
            }
            ...
            for (const textLine of textLines) {
                ...
                for (let i = 0; true; i++) {
                    if (getTextWidth(textObj, wordsInLine[i] || "", false) > maxWidth) {
                        ...
                        if (i === 0 && prevLine) {
                            if (nextLine) { wordsInLine = [prevLine, curLine, nextLine, ...wordsInLine.slice(2)] } else { wordsInLine = [prevLine, curLine] }
                            i++;
                        } else {
    ...
    const justifyText = (textRef, justification, maxWidth = 0) => {
        ...
        if (VAL({textObj})) {
            ...
            setText(textObj, null, null, true);
    ...
    const regText = (textRef, hostName, activeLayer, hasShadow, justification = "center", options = {}, funcName = false) => {
        ...
        if (VAL({text: textObj})) {
            ...
            let name;
            if (options.name && !REGISTRY.TEXT[options.name]) { name = options.name } else if (!REGISTRY.TEXT[hostName]) { name = hostName } else { name = `${hostName.replace(/(_|\d|#)+$/gu, "")}_${_.filter(Object.keys(REGISTRY.TEXT), (k) => k.includes(hostName.replace(/(_|\d|#)+$/gu, ""))).length + 1}` }
            ...
            setTextData(textObj, REGISTRY.TEXT[name]);
            ...
            if (options.isActive === false) { toggleText(name, false, true) } else { toFront(textObj) }
    ...
    const setText = (textRef, text, isToggling, isForcing = false) => {
        ...
        if (!isForcing && (text === null || text === undefined)) {
            return TRACEOFF(traceID, null);
        } else if (isForcing && (text === null || text === undefined)) {
            text = (getTextObj(textRef) || {get: () => ""}).get("text");
        }
        const textData = getTextData(textRef);
        if (VAL({list: textData}, "setText")) {
            ...
            if (VAL({textObj}, ["setText", `textRef: ${D.JS(textRef)}, text: ${D.JS(text)}`])) {
                ...
                const setParams = D.Clone(textParams);
                if (setParams.left === textObj.get("left")) { delete setParams.left }
                if (setParams.top === textObj.get("top")) { delete setParams.top }
                trySet(textObj, setParams);
                if (textData.shadowID) { updateTextShadow(textKey) }
                if (textData.linkedText) { updateSlaveText(textKey) }
                const curWidth = getTextWidth(textKey, textParams.text, textData.maxWidth);
                const curHeight = getTextHeight(textKey, textParams.text, textData.maxWidth);
                REGISTRY.TEXT[textData.name].width = curWidth;
                REGISTRY.TEXT[textData.name].height = curHeight;
                REGISTRY.TEXT[textKey].leftEdge = textParams.left - 0.5 * curWidth;
                REGISTRY.TEXT[textKey].rightEdge = textParams.left + 0.5 * curWidth;
                REGISTRY.TEXT[textKey].topEdge = textParams.top - 0.5 * curHeight;
                REGISTRY.TEXT[textKey].bottomEdge = textParams.top + 0.5 * curHeight;
                return TRACEOFF(traceID, textObj);
            }
        }
        return TRACEOFF(traceID, false);
        ...
        if (VAL({string: textKey}, "setTextData")) {
            ...
            if (VAL({textObj}, ["setTextData", `Registered object '${textKey}' not found!`])) {
                ...
                _.each(textParams, (v, k) => {
                    if (k === "text") { D.Alert("Attempt to set 'text' via setTextData: Use setText() to set text values!", "ERROR: setTextData") } else { REGISTRY.TEXT[textKey][k] = v }
                });
                trySet(textObj, _.omit(textParams, "text"));
                if (_.intersection(Object.keys(textParams), ["shiftTop", "top", "shiftLeft", "left", "pushtop", "pushleft"]).length) { setText(textKey, null, undefined, true) }
    ...
    const toggleText = (textRef, isActive, isForcing = false) => {
        ...
        if (VAL({list: [textData, modeData]}, "toggleText", true)) {
            let activeCheck = null;
            if ((isActive === true || (isActive !== false && !textData.isActive)) && modeData.isForcedOn !== "NEVER") { activeCheck = true } else if (isActive === false || (isActive !== true && textData.isActive) || (modeData.isForcedOn === "NEVER" && textData.isActive)) { activeCheck = false }
            ...
            if (activeCheck === false) {
                ...
                if (textData.shadowID) { trySet(getObj("text", textData.shadowID), {layer: "walls"}) }
                return TRACEOFF(traceID, false);
            } else if (activeCheck === true) {
                // TURN ON: Set layer to active layer, toggle on associated drag pads, restore activeState value if it's different
                ...
                if (textData.shadowID) { trySet(getObj("text", textData.shadowID), {layer: textData.activeLayer}) }
                return TRACEOFF(traceID, true);
            }
        }
        return TRACEOFF(traceID, null);
    ...
    const clearUnregText = (isKilling = false, isKillingPlayerText = false) => {
        ...
        for (const textObj of unregTextObjs) {
            const textLayer = textObj.get("layer");
            if (textObj.get("pageid") !== C.PAGES.GAME
                    || (!isKillingAll && textLayer === "gmlayer")) { continue }
            returnLines.push(
            ...
            if (isKillingAll || (isKillingNonGMLayer && textLayer === "gmlayer")) { textObj.remove() }
        }
    ...
    const fixTextObjs = (isQueueing = false) => {
        ...
        for (const [textData, textObj] of textPairs) {
            ...
            if (textData.curText !== textObj.get("text")) {
                if (textData.curText && VAL({string: textData.curText})) {
                    reportStrings.push(
        ...
        if (reportLines.length) {
            if (isQueueing) { STATE.REF.fixAllCommands.push(...["<h3><u>Final Text Object Pass</u></h3>", ...reportLines]) } else { D.Alert(["<h3><u>Fixing Text Objects</u></h3>", ...reportLines].join("<br>"), "fixTextObjs") }
        }
        TRACEOFF(traceID);
    };
        // #endregion
