(() => {
    ...
    */
    ...
    on(getTriggers(ALLATTRS, "", [..._.keys(DISCREPREFS), ..._.keys(ADVREPREFS)]), (eInfo) => {
        if (!ISOFFLINE && !isBlacklisted(eInfo.sourceAttribute)) {
            ...
            log(`[${eInfo.sourceAttribute}, ${eInfo.sourceType}] @@@ MOD DOTS TRIGGERED (ROLL) @@@`);
    ...
    */
    ...
    const updateModDots = (sourceAttr) => {
        ...
        log(`STATSLIST: ${JSON.stringify(statsList)}\n\nREPSTATDATA: ${JSON.stringify(repStatData)}`, "UMD");
        const repSecs = Object.keys(repStatData);
        const getRepAttrs = (repSec) => {
            if (repSec)
                getSectionIDs(repSec, (rowIDs) => {
                ...
                getAttrs([..._.flatten(statsList), ..._.flatten(repAttrs)], (ATTRS) => {
                    log(`FULL ATTRS: ${JSON.stringify(ATTRS)}`, "UMD");
                    ...
                    if (_.isNaN(modDelta)) {
                        log(`ERROR: modDelta '${JSON.stringify(ATTRS[modStat])}' parses to '${JSON.stringify(modDelta)}' --> not a number.`, "UMD");
                        return;
                    }
                    if (modStat.includes("health")) {
                    ...
                    log(`DELTA ATTRS: ${JSON.stringify(attrList)}`, "UMD");
                    setAttrs(attrList);
                });
        };
        getRepAttrs(repSecs.shift());
    ...
    on("change:repeating_advantage", (eInfo) => {
        log(`[TRIGGER: ${JSON.stringify(eInfo)}]`);
    });
    // #endregion
